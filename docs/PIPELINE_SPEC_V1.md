...
**SUPERSEDED IN PART by §16.46 — the `mask_refine_mode` line in the `[text_detector]` block below.** That line originally read `"simple"` with a comment naming `Simple` as the default; item 5 of that entry required both halves rewritten, and the line below now carries the shipped value `"annotation"` with `"simple"` as the opt-out. Nothing else in this block is changed by that entry.
...
mask_refine_mode             = "annotation"  # simple | annotation (Annotation is the shipped default; `simple` is the opt-out). `simple` is a deliberate v1 addition, not present upstream.
...
- **Out of scope for v1:** upstream's `refine_mask` (top-k colour masklists + per-channel Otsu candidates + connected-component XOR merge + hole filling, `comic_text_detector/utils/textmask.py:18-210`) and `refine_undetected_mask`. Rationale: the masker's own growth-and-score loop is what determines final quality; a coarser precise mask degrades fit granularity but cannot break the pipeline, and the XOR-merge algorithm is the single riskiest numerical port in the whole project. Keep the door open with `enum MaskRefineMode { Simple, Annotation }` in `TextDetectorConfig` where `Annotation` is the opt-in upstream refinement path and `Simple` remains the default. See §15.2.
  ...
  **SUPERSEDED IN PART by §16.46 — the "`Annotation` is the opt-in upstream refinement path and `Simple` remains the default" clause only.** Both halves invert: `Annotation` is the shipped default and `Simple` is the opt-out. The rest of this bullet is a historical record of why the two upstream functions were out of scope for v1, and stays exactly as written.
...
12. **Detector mask refinement** — v1 ships the "Simple" refinement, not upstream's `refine_mask`/`refine_undetected_mask` (§15.2).
    ...
    **SUPERSEDED by §16.46 — `DEVIATION(12)` is RETIRED.** The default-value divergence that was this entry's only remaining content is gone: the shipped default is now `Annotation`, which is upstream's own unconditional refinement. The entry is kept here, retired and dated 2026-08-10, so a future reader sees that the rule changed rather than that it never existed.
...
17. **Coverage-filter scope and operand** — ratified by §16.20 item 9, which corrects §8.3 step 6's former claim of unconditional parity. Upstream applies its `mask_score < mask_score_thresh` false-positive filter **only to line-less blocks** (`textblock.py:485-490`, inside `if len(blk.lines) == 0:`) and computes it over the **unrefined** mask (`inference.py:203` passes `mask` to `group_output`; `refine_mask` runs at `:204`). v1 applies the filter to **every** block over the **refined** mask. Both differences are deliberate: v1 synthesizes no DBNet line polygons (§14.12 and §8.3's out-of-scope list), so every block is line-less by construction and the scope difference is vacuous *for v1* — it would become live the moment line synthesis lands, which is why it is registered rather than left as prose. The operand difference makes our coverage values roughly 2× upstream's on the same boxes; measured across two real manga pages the filter has never fired (minimum coverage 0.3025 against a 0.1 threshold). The implementation site carries `DEVIATION(17)`.
    ...
    **SUPERSEDED IN PART by §16.46 — the clause above reading "and is still the shipped default" only.** Under the new default the coverage filter scores the unrefined mask, which is upstream's own operand, so this item's OPERAND divergence does not apply to a default run. Its SCOPE half (every block, not only line-less ones), its 0.1 threshold, and `DEVIATION(17)`'s implementation site are all untouched.
...
30. **`inpainting_enabled` ships `true`, where upstream's dataclass default is `False` — ratified by §16.46 items 1(b) and 10 (maintainer product decision, 2026-08-10).** Upstream's runtime authority for the `[inpainter]` block is `config.py:817-824` (§16.38 item 7(b)), and `inpainting_enabled = False` is the first of those eight values. §16.38 item 13(a) adopted all eight verbatim; this port now diverges on that one, deliberately, so that a default run inpaints instead of flat-filling. **The other seven values are unchanged and are still upstream's.** The implementation site is `InpainterConfig::default()` in `crates/pc-config/src/profile.rs` and carries the `DEVIATION(30)` comment; no line number is given here, for the reason §16.39 item 2(c) gives for giving none. **Two consequences travel with this entry rather than being inferred from it:** the LaMa artifact is `Requirement::Required` (§16.46 item 11(b)), because a default-on stage whose model is absent aborts the batch; and §16.38 item 9(f)'s containment sentence — *"The blast radius is confined to users who explicitly set the flag, since it defaults to false"* — no longer holds, which is what forced that promotion. **Symmetry worth stating, since the two halves of §16.46 pull opposite ways:** the mask-mode half of that entry RETIRES item 12 by adopting upstream's algorithm as the default, while this half CREATES a divergence by declining upstream's default value.
...
2. **`MaskRefineMode::Simple` for v1** (§8.3 step 5) — **DECIDED: ship Simple, do not port upstream's full `refine_mask`/`refine_undetected_mask`.** Verified the full algorithm (`textmask.py:18-214`): top-k grey/Otsu masks + XOR-minimizing merge + hole filling — real work, correctly flagged as the riskiest port in the project. Deciding evidence: the `demo_bubbles` fixtures live in `media/` (README demo assets) and are **never used by upstream's own test suite** — their exact producing version/profile is unverifiable. Porting the riskiest algorithm in the project to chase parity with fixtures of unknown provenance is a bad trade. **Consequence (not a contingency — the plan of record):** §10.7(B) item 15's upstream-image comparison is downgraded from a frozen gate to a **non-gating calibration report** (see §10.7(B) rewrite below); the frozen masking test is a regression lock against our own recorded-fixture pipeline output, calibrated by F2. `Annotation` mode is now available as an opt-in for a full refinement port.
   ...
   **SUPERSEDED IN PART by §16.46 — the "`Simple` is what v1 ships by default" half only.** The downgrade of the upstream-image comparison to a non-gating calibration report, declared in this item's own Consequence sentence above, is NOT touched. That downgrade's surviving ground is this item's own point about the `demo_bubbles` fixtures' unverifiable provenance, which is a fact about the fixtures and not about our refinement algorithm; a reader who takes the un-ported-algorithm ground as the only one will restore a parity gate that both entries refuse.
...
9. **Scope quarantine, so no gate moves under this work.** `Annotation` ships **non-default**; `Simple` stays the default and every value frozen against it stays put. Adding a second recorded page or a new fixture group is **not** in A0–A5: `xtask/src/record.rs`'s `detector_plan` is single-page by §16.29 item 2, and `crates/pc-testkit/tests/recorded_provenance.rs` carries a hard-coded group count and hard-coded artifact paths. That is A6's problem, under its own ruling.
   ...
   **SUPERSEDED IN PART by §16.46 — the first two propositions only.** `Annotation` now ships as the default and `Simple` does not. The third — that every value frozen against `Simple` stays put — is honoured there by mechanism rather than by coincidence: each such test names `Simple` explicitly in its input instead of inheriting it from the default.
...
9. **RULING — a missing or uninitializable inpainting model is RUN-FATAL. This agrees with the architect's conclusion and rejects the architect's ground.**
   ...
   **SUPERSEDED IN PART by §16.46 — sub-item (f)'s containment sentence only.** The flag no longer defaults to false, so the blast radius is no longer confined to users who set it. That entry resolves the consequence by promoting the LaMa artifact to `Requirement::Required`, not by weakening this item's run-fatal ruling, which stands.
...
13. **The `[inpainter]` config surface — eight keys, transcribed from `config.py:817-824`.**
    ...
    **SUPERSEDED IN PART by §16.46 — sub-item (a)'s FIRST value only.** `inpainting_enabled` now ships `true` and is registered as `DEVIATION(30)`. The other seven values in (a) are unchanged and are still upstream's dataclass defaults, and (b) through (e) are untouched.
...
15. **§16.23 item 5's sequence is REVISED: LaMa moves ahead of GPU-2 and ahead of the INI-import + Lab-NLM batch.**
    ...
    **SUPERSEDED IN PART by §16.46 — sub-item (e)'s PREMISE only.** `inpainting_enabled` no longer defaults to false. The conclusion that no committed fixture moves is retained there and re-grounded on the recorders never constructing an `InpainterConfig` at all; everything else in this item, including the sequencing amendment itself, is untouched.
...
2. **DECIDED: `DEVIATION(12)` is narrowed, not retired — and this contradicts the word "retirement" in §16.37 items 8 and 11(d).** Both planning agents reached this independently, with the same argument, having each noticed that the existing ratified text says "retirement".
   ...
   **SUPERSEDED by §16.46 — the retirement this item argued against is now the ruling.** Read that entry before citing this item's "narrowed, not retired" decision as current. Its analysis of why the entry could not be retired in the tree as it stood on 2026-08-07 holds as written, and is the reason the retirement needed a separate product decision rather than following from A4.
...
6. **D4 — Device disclosure: one process-wide statement (architect's design wins),
   ...
   **SUPERSEDED IN PART by §16.47 item 6 — read it before citing this graft's
   detector-mechanism sentence as unconditional.** Once the text detector's
   constructor takes a real `&DevicePolicy` and attempts registration (§16.47 item 3),
   the sentence *"its constructor takes no device argument at all … never as a
   registration"* is false. The inpainter sentence, the one-process-wide-statement
   ruling, and the per-stage-row shape are untouched and still apply.
...
## 16.46 The shipped defaults become `mask_refine_mode = "annotation"` and `inpainting_enabled = true` (maintainer product decision; joint architect + Senior Rust Engineer plan pass; Fable tie-break on fixture scope and on the A5 precondition, 2026-08-10)

**Provenance, stated as fact rather than as framing, because a provenance claim is checked like any other.** Three distinct acts by three distinct parties, all on 2026-08-10:

(i) The **maintainer** made the product decision — ship both new defaults, CPU-only, fully user-overridable — and separately chose, on being shown that a default-on inpainter whose model is optional aborts a fresh install's first eligible page, to promote the LaMa artifact to `Requirement::Required`. Neither is open for re-litigation here.

(ii) The **architect** and the **Senior Rust Engineer** planned the implementation in parallel. They converged on the two shipped values, the opt-out contract, the frozen-test change list and the supersession target list, and they disagreed on exactly two points.

(iii) **Fable** ruled on those two points as tie-breaker. Both rulings are quoted verbatim in items 8 and 9 and are transcribed from `docs/briefs/default_annotation_lama_implementation_brief.md`, the record of that dispatch. The transcriber did not witness the deliberation and therefore quotes rather than paraphrases.

**What this entry may NOT cite as its ground.** No measurement in this repository ranks the four mode combinations. `docs/MODE_COMPARISON.md` section 6 reads, verbatim: *"This report ranks nothing and asserts nothing. It records what each cell measured on the same inputs so a maintainer can read the differences directly. Cells are not ordered by their distance to any reference image, and no cell is declared better or worse than another here."* No clause here may offer `cargo xtask mode-bench` as evidence that the new defaults produce better output, and §16.38 item 17(b)'s decision point D2 is untouched and still open. The ground for this change is a product decision and is stated as one.

1. **The two values, and the opt-out contract.**

   (a) `MaskRefineMode::default()` is `Annotation`.

   (b) `InpainterConfig::default().inpainting_enabled` is `true`.

   (c) Both stay user-overridable. The opt-outs are `mask_refine_mode = "simple"` in a profile, `inpainting_enabled = false` in a profile, and the per-run `--skip-inpaint` flag. **This decision is what makes the opt-out path load-bearing**: until now those three values equalled the shipped defaults, so no test exercising them could have failed. Item 13 requires a frozen test on each.

   (d) CPU stays the only default execution provider. Nothing here touches `[general] device`.

2. **`DEVIATION(12)` is RETIRED, not narrowed a second time.**

   **SUPERSEDES: §14 item 12**

   **SUPERSEDES: §16.39 item 2**

   (a) **The scope being retired, quoted verbatim from §16.39 item 2(b) so it travels with the retirement:** *"the shipped default is `Simple`, so a default run diverges from upstream's unconditional refinement; parity is reachable only by opting in with `mask_refine_mode = "annotation"`."* That default-value divergence is the entry's whole remaining content — §16.39 item 2(b) says so in the same breath: *"The divergence is real, it is a default-value divergence, and it is the only one left at this site."* Item 1(a) removes it, so the register entry is retired in place, marked and dated, never deleted.

   (b) **What the retirement does NOT claim.** It does not claim that this port has reached parity with upstream in every respect. What it claims is narrower: the *default-value* divergence is gone. Our continuing to offer a `Simple` mode, which upstream has no equivalent of, is untouched by the retirement and is not asserted to be parity. Upstream has no mode switch at all (`inference.py:204` runs `refine_mask` unconditionally), so the continued existence of `Simple` as a selectable non-default mode is a v1 addition, recorded today as prose at §6's `mask_refine_mode` line: *"Deliberate v1 addition, not present upstream."* Whether that addition needs its own register entry is left open by item 14 and is not decided here.

   (c) **The site comment.** `DEVIATION(12)`'s primary comment sits on the `#[default] Simple` variant of `MaskRefineMode` in `crates/pc-config/src/profile.rs`, where §16.39 item 2(c) moved it. On retirement the comment goes with the entry: the variant is no longer the default and no longer carries the divergence. No line number is given here, for the reason §16.39 item 2(c) gives for giving none — the implementing task edits that file in the same commit.

3. **§15 item 2's default half stops holding; its calibration-report half does not, and the difference is the whole point of this item.**

   **SUPERSEDES: §15 item 2**

   (a) **What stops holding**, quoted verbatim: *"`Simple` is what v1 ships by default"* (the wording is §16.39's re-statement of §15 item 2's decision, at that item's own back-pointer line).

   (b) **What does NOT stop holding: the downgrade of §10.7(B) item 15's upstream-image comparison to a non-gating calibration report.** §15 item 2 gave that downgrade two independent grounds, and only the first of them moves. The surviving ground, quoted verbatim: *"the `demo_bubbles` fixtures live in `media/` (README demo assets) and are **never used by upstream's own test suite** — their exact producing version/profile is unverifiable. Porting the riskiest algorithm in the project to chase parity with fixtures of unknown provenance is a bad trade."* That is a property of the fixtures, not of our refinement algorithm, so shipping upstream's refinement by default is **not** a licence to restore a frozen parity gate against `_clean.png`. A reader who takes the first ground as the only one will draw exactly the wrong conclusion, which is why both are written out here.

4. **§8.3 step 5's out-of-scope bullet no longer describes the shipped configuration.**

   **SUPERSEDES: §8.3 step 5**

   The clause, verbatim: *"where `Annotation` is the opt-in upstream refinement path and `Simple` remains the default."* Item 1(a) inverts both halves: `Annotation` is the default and `Simple` is the opt-out. Everything else in that bullet — the description of what `refine_mask` and `refine_undetected_mask` do, and the record of why they were out of scope for v1 — is a historical statement about v1 and stays exactly as written.

5. **§6's shipped default TOML block carries the old value and the old comment.**

   **SUPERSEDES: §6**

   The `[text_detector]` line, verbatim: `mask_refine_mode             = "simple"  # simple | annotation (Annotation is opt-in; Simple remains the default). Deliberate v1 addition, not present upstream.` The value becomes `"annotation"` and the parenthetical is rewritten so that the comment and the value beside it agree; the *"Deliberate v1 addition, not present upstream"* clause stays true and now describes `Simple`. §6 carries no `[inpainter]` block — it is explicitly *"minus `[inpainter]` (v1.5)"* — so `inpainting_enabled`'s shipped value is pinned by item 10 and not here.

   **This marker is the transcriber's own addition, disclosed as such.** Neither planning agent's target list named §6; both stopped at the `[text_detector]` clauses in §8.3 and §14. The shipped default block in §6 states the old value in normative text, so it is added here on the same reasoning §16.39 item 1 used for its own additions — and it is recorded as a transcription-time act rather than presented as pre-existing, because who decided a thing is part of the record.

   **A property of this back-pointer, measured rather than assumed.** §6's span carries exactly one `§16.46` mention — the note this item requires — so that note is load-bearing: removing it turns `every_supersession_marker_has_a_back_pointer_at_its_target` red, confirmed by doing it on 2026-08-10 and restoring. §6's span also carries exactly **one** `§16.38` mention, so the older `("16.38", "6")` row is load-bearing in exactly the same way and by the same measurement, not less so. Both counts are facts about §6's current text and not properties of the rows: a later edit adding a second citation of either section anywhere in §6 would quietly make that row unfalsifiable, and nothing would notice.

6. **§14 item 17's operand note is narrowed, and its scope half survives.**

   **SUPERSEDES: §14 item 17**

   (a) The clause that stops holding, verbatim: *"describes `MaskRefineMode::Simple`, which is still exactly what it says and is still the shipped default."* The first half stays true — that sentence does describe `Simple` — and the second half does not.

   (b) **The consequence, stated because it is easy to miss and changes what a default run does.** Under the new default the coverage filter scores the **unrefined**, letterbox-cropped, resized detector mask, which is upstream's own operand. So `DEVIATION(17)`'s **operand** divergence does not apply to a default run at all. Its **scope** half — the filter applied to every block rather than only to line-less ones — and the `0.1` threshold are untouched, and `DEVIATION(17)`'s implementation site is untouched. This item narrows one clause of item 17; it does not retire item 17.

7. **§16.38 item 15(e)'s premise stops holding, and its conclusion is re-grounded on a fact that no default value can move.**

   **SUPERSEDES: §16.38 item 15(e)**

   (a) The clause, verbatim: *"**The load-bearing half: LaMa implies no fixture re-record.** `inpainting_enabled` defaults to `false` (`config.py:817`, item 13(a)), so no default-path artifact changes and no committed fixture moves."* Item 1(b) makes the premise false.

   (b) **The re-grounding, written out rather than assumed, because it is the reasoning that makes this item correct rather than merely convenient.** The conclusion — the `inpainting_enabled` flip forces no fixture re-record — is retained, and it now rests on the **structural independence of the recorders from `InpainterConfig`**, not on the flag's value. Measured 2026-08-10 by reading the code, not inferred: `git grep -c inpaint -- xtask/src/record.rs` returns **no hits at all** (no output, exit 1 — `-c` prints nothing for a file with zero matches rather than printing `0`), so the detector fixture recorder constructs no `InpainterConfig`, calls nothing in `pc-inpaint`, and never reaches `pc_pipeline::run_inpaint`; and `xtask/src/calibrate.rs`'s stage chain is `pc_detect::run` → `pc_preprocess::run` → `pc_mask::run` → export, with no inpainting stage anywhere in it. Neither recorder can observe `inpainting_enabled` at any value. That is a stronger ground than the one it replaces, because a default value is exactly the kind of thing a product decision moves and a missing code path is not.

   (c) **The scope of the re-grounding, stated so it is not read wider than it is.** It covers the `inpainting_enabled` flip and nothing else. It does **not** cover the `mask_refine_mode` flip, which item 8 shows is not artifact-neutral. Reading (b) as a statement about "the flip" rather than about "the `inpainting_enabled` flip" would license exactly the silent fixture drift item 8 exists to prevent.

8. **Fixture scope for the mask-mode flip — RULED (Fable tie-break, 2026-08-10). This is the first of the two points the architect and the Senior Rust Engineer disagreed on.**

   (a) **The ruling, quoted verbatim and in full:**

   > **Decision.** Pin the three xtask sites to `MaskRefineMode::Simple` and add the two tripwire tests (pin-differs-from-default; enumerated-and-reasoned bare-default census). Do **not** add `_raw_mask_annotation.png` / `#raw_annotation.json` in this task.
   >
   > **Grounds.** (1) The architect under-costed committing the artifact: producing it is cheap, but committing it means editing `recorded_provenance.rs`'s frozen declared/committed/exempt path-set constants and digest rows, and `PROVENANCE.json` is generator-produced from a fixed key list (§16.37 item 10(f)) — a hand-added artifact declaration is dropped at the next re-record. (2) The premise "the default path is completely unguarded by any committed fixture" is materially false: `a4_run_annotation.rs:372` already gates the Annotation output of the recorded page against literals (nonzero count + positional fingerprint + dimensions + survivor rects) — a regression that preserves all four simultaneously is a fingerprint-collision class, not a realistic defect, and the path is deterministic (verified), so that pin is meaningful. (3) With the A5 ruling below, the remaining gap closes.
   >
   > **Graft from the losing position (keep these):** the architect's PROVENANCE.json gaining `detector.ours.mask_refine_mode` — added **through the generator's key list**, not by hand (per item 10(f)) — and report generators rendering the mode from the config they actually ran, not a hard-coded literal. The rust-engineer's `profile_non_default` fix is the same defect class and lands too.
   >
   > **Scope.** This does not decide the architect's F-a ("re-record the primary fixture under Annotation as the honest long-term end state") — that remains a separate, later, ratified task per §16.38 item 15(e).

   (b) **Why a pin is needed at all, with the measurement that forced it.** `pc_detect::run` branches on `mask_refine_mode` for both the mask it writes to `raw_mask_dest` and the operand the coverage filter scores (`crates/pc-detect/src/lib.rs:112-137`). Three xtask sites read `TextDetectorConfig::default()` and feed it straight into that function: `xtask/src/record.rs:654`, `xtask/src/calibrate.rs:937` and `:984`, and `xtask/src/mask_sweep.rs:280`. Measured 2026-08-10 on a throwaway worktree at `40e2b5a`: flipping the two defaults alone takes `cargo test --workspace --no-fail-fast` from **1418 passed / 0 failed / 3 ignored** to **1409 passed / 9 failed / 3 ignored**, and two of the nine are behavioural rather than value pins — `xtask/tests/mask_sweep_cli.rs:76` reports a changed ladder, and `crates/pc-detect/tests/d7_run.rs:628` loses equality with the committed `#raw.json`. The pin is what keeps the committed artifacts reproducible from a stated configuration.

   (c) **`xtask/src/mask_sweep.rs:208` is deliberately NOT pinned.** It passes the config to `OnnxDetector::from_path_with_config`, which reads the thread and path fields and never reads `mask_refine_mode`. Pinning a site that cannot observe the value would make the census in item 8(e) harder to read for no gain.

   (d) **The provenance fixes, both of which are the same defect class.** `xtask/src/record.rs:798` writes `"profile_non_default": {}` as a hard-coded literal, while `crates/pc-testkit/src/provenance.rs` documents that field as *"built by diffing against the profile defaults"*. Once the recorder runs under an explicit non-default mode, that literal is a false claim about the recording and must be derived from the resolved config. Separately, `detector.ours.mask_refine_mode` is added through the generator, and it is added to the typed `OursPins` struct as well, which carries `#[serde(deny_unknown_fields)]` — so writer and reader share one definition and a future mismatch is a parse error naming the field rather than a silent absence. That is cookbook rule 14's hand-indexing lesson applied at the point where the format changes.

   (e) **The two tripwire tests the ruling names.** The first asserts that the pinned recording mode is the literal `Simple` **and** that it differs from the shipped default — the second assertion is what makes the pin load-bearing rather than decorative, and it is an assertion that could not have failed before this entry. The second enumerates every remaining bare `TextDetectorConfig::default()` under `xtask/src/` and requires each to be declared with a reason it produces no committed artifact, asserted as a set of `(file, count)` pairs rather than a total, because a total is satisfied by a swap. The baseline that census starts from was measured 2026-08-10 with `git grep -c "TextDetectorConfig::default()" -- xtask/src/`, read to the end: `calibrate.rs` 2, `mask_sweep.rs` 2, `mode_bench.rs` 3, `record.rs` 1 — **eight occurrences in four files**. `xtask/src/device.rs` is **not** among them: its `TextDetectorConfig` at `:80` is a function parameter, not a `::default()` call. One planning draft listed it as a site; it is not one, and the census must be re-measured after the pins land rather than derived from this sentence.

   **AMENDED, Orchestrator ruling following D1's independent review, 2026-08-10.** As written above, the first tripwire's second assertion cannot be satisfied: it requires a runtime comparison showing the pin differs from the shipped default, while item 15 requires D1 to be green **both before and after** the flip — and before the flip the pin and the default are the same value, so the comparison fails exactly when item 15 says it must not. The ruling substitutes the technique and keeps the intent. The second assertion becomes a **source-level** check: the definition of `RECORDING_MASK_REFINE_MODE` must be a literal variant and must not be an expression that reads `TextDetectorConfig::default()`. That is the same class of check this item's own census tripwire already performs — it reads source structure rather than a runtime value — it preserves what the original assertion was for (the pin must be provably independent of whatever the default happens to be, not decorative), and it is green on both sides of the flip because it never compares against a value that moves. The first assertion, that the pinned mode is the literal `Simple`, is unchanged. **This is a testing-technique substitution, not a change to what is being asserted about the system**, which is why it lands as an Orchestrator ruling rather than a fresh joint-architect round.

9. **The A5 determinism gate becomes a precondition for the mask-mode flip — RULED (Fable tie-break, 2026-08-10). This is the second of the two disputed points.**

   (a) **The ruling, quoted verbatim and in full:**

   > **Decision.** The flip does not land until `crates/pc-detect/tests/a5_*.rs` exists and is green: a frozen test driving `pc_detect::run` under `MaskRefineMode::Annotation` on the recorded page via `ReplayDetector`, asserting structural identity across repeated runs and at least two rayon thread counts, plus disk-artifact byte-identity across runs — the shape of `d7_run.rs:558`, retargeted at Annotation. The **calibration document** (non-gating upstream comparison with CPU-feature/IPP pins) is **not** a precondition; it stays a tracked backlog item.
   >
   > **Grounds.** The spec itself (§16.37 item 3) ranks these as two artifacts of different rank: a frozen gate and a non-gating report. The CPU-feature/IPP pins attach to the *report*, not the *gate*. Verified empirically: 10 sequential runs + 24 pooled runs (1/2/8 threads) all byte-identical, disk artifact byte-identical, no ONNX model needed, runs in seconds — the precondition costs almost nothing.
   >
   > **Minimal-A5 scope.** One gate through `pc_detect::run` suffices — no per-function split. Both `refine_mask` and `refine_undetected_mask` are covered because `run`'s Annotation branch composes them (`lib.rs:118-126`). One honest limit: on the recorded page the undetected pass invents no block (degenerate case), so the recursion-over-invented-blocks branch executes its scan-and-filter but not its recursive refine — this residual is inherent to A5 as ratified (item 3 scopes the gate to the recorded page; a second page is quarantined to A6 by item 9) and does not widen this precondition. `a3b_annotate_refine.rs`'s synthetic-input coverage remains the guard for that branch.
   >
   > **Scope.** This makes the A5 *gate* a precondition for flipping `mask_refine_mode`'s default — not for the `inpainting_enabled` flip, which can sequence independently.

   (b) **Three citations across the two rulings above were re-checked against the tree at `40e2b5a` while transcribing them, rather than carried over. The first belongs to item 8's ruling, not to this one**, and is verified here so that all three sit together. `crates/pc-detect/tests/a4_run_annotation.rs:372` — cited by item 8(a)'s ground (2) — is the `fn` line of `run_in_annotation_mode_on_the_recorded_page_produces_the_expected_block_rects`, and its four literals are the survivor rectangles, `mask.dimensions() == (1200, 1660)`, `nonzero_count == 2_942` and `positional_fingerprint == 2_790_551_277`. `crates/pc-detect/tests/d7_run.rs:558` is the `fn` line of `a6_pending_recorded_page_equality_and_determinism`. `crates/pc-detect/src/lib.rs:118-126` is the `MaskRefineMode::Annotation` arm that composes `refine_mask` and `refine_undetected_mask`. All three hold.

   (c) **The calibration document stays owed.** It is not a precondition, and it is not cancelled: the non-gating upstream comparison with the CPU-feature-set and IPP pins that §16.37 items 3 and 10 require remains a tracked backlog item, and no clause here shortens what those items ask of it.

10. **DEVIATION(30) — `inpainting_enabled = true` diverges from upstream's own dataclass default.**

    **SUPERSEDES: §16.38 item 13(a)**

    For the first of that item's eight values only. The clause, verbatim: *"`inpainting_enabled = false`, `inpainting_min_std_dev = 15.0`, `inpainting_max_mask_radius = 6`, `min_inpainting_radius = 7`, `max_inpainting_radius = 20`, `inpainting_radius_multiplier = 0.2`, `inpainting_isolation_radius = 5`, `inpainting_fade_radius = 4`. Every value is `config.py`'s dataclass default, which item 7(b) established is the runtime authority; the disagreeing values in `media/default.conf` are documentation and are not used."* The other seven values are unchanged and are still upstream's; only `inpainting_enabled` moves, and because it now differs from `config.py:817` deliberately it is registered as **`DEVIATION(30)`** with its comment at the `InpainterConfig::default()` site. Note the symmetry, since it is the clearest one-line summary of this entry: the mask-mode flip **retires** a deviation by adopting upstream's algorithm as the default, and the inpainting flip **creates** one by declining upstream's default value.

11. **§16.38 item 9(f)'s containment sentence stops holding, and the consequence is resolved rather than left open.**

    **SUPERSEDES: §16.38 item 9(f)**

    (a) The sentence, verbatim: *"The blast radius is confined to users who explicitly set the flag, since it defaults to false."* Item 1(b) makes it false, and it is load-bearing: it is the sentence on which item 9 accepted that a missing or unloadable model aborts the whole batch with zero pages exported and exit 1.

    (b) **The resolution the maintainer chose: `LAMA_MANGA_INPAINTER` is promoted from `Requirement::Optional` to `Requirement::Required`** (`crates/pc-models/src/lib.rs:92`). A fresh install's `panel-ocr models download` therefore fetches the 207,482,644-byte artifact without a flag, `models verify` reports it as a required row, and the failure item 9(f) contemplated becomes unreachable for a user who has provisioned normally. Two other options were put to the maintainer and declined: leaving the model optional and accepting a run-fatal first run, and degrading to a flat fill with a WARN. The third of those directly contradicts item 9's *"The remedy is provisioning, not a runtime fallback"* and would need its own ratification; it is not adopted here, and it is materially the flat-fill backlog idea already recorded on this branch.

    (c) **Consequences that are part of this decision and not side effects.** §16.38 item 19's cost argument for optionality — *"Adding the inpainting spec to `ALL` grows every user's `models download` by 207,482,644 bytes for a feature that defaults off"* — is spent, because the feature no longer defaults off; the partition machinery `ALL`/`selected`/`skipped` and the `--include-optional` flag stay in place and simply have no member on the optional side of this artifact. `crates/pc-models/src/lib.rs:84`'s justification prose is false as written and is corrected with the promotion. `crates/pc-models/tests/d1_resolve.rs`'s `the_required_optional_partition_is_pinned_by_name` changes value by design — that test exists to force exactly this to be a deliberate edit — and it stays a **set** comparison, never a count.

12. **§16.37 item 9's quarantine clause: the first half stops holding and the second half is honoured by mechanism rather than by luck.**

    **SUPERSEDES: §16.37 item 9**

    The clause, verbatim: *"`Annotation` ships **non-default**; `Simple` stays the default and every value frozen against it stays put."* Item 1(a) falsifies the first two propositions. **The third is honoured, and item 13(b) is how**: every test whose subject is `Simple`'s behaviour names `Simple` explicitly in its input, so every value frozen against `Simple` does stay put — it stays put because a test says which mode it means, not because the default happens to agree with it. That is a strictly better state than the one this clause described, and it is the state item 8's pins put the recorders in as well.

13. **Pre-authorised frozen-test changes, scoped to exactly two classes. Anything else returns to both architects.**

    The precedent is §16.38 item 11's three pre-authorised value corrections. The two classes:

    (a) **Value corrections where the assertion's subject IS a shipped default.** Six sites, each re-derived at `40e2b5a`: `crates/pc-config/tests/defaults.rs:170`, `:379` and `:427`; `crates/pc-inpaint/tests/l4_eligibility.rs:213`; `crates/pc-detect/tests/a4_run_annotation.rs:230` (a premise assertion that the default is `Simple`, inside a test whose own subject is `Simple` mode and which already passes the mode explicitly); and `crates/pc-config/tests/round_trip.rs:363`. The last of the six is called out separately because it is the only one where the *meaning* moves rather than the value: that test edits `inpainting_enabled` in the direction whose loss is invisible, and after item 1(b) the invisible loss is a dropped `false` rather than a dropped `true`, so the edited direction inverts. Both planning agents reached that conclusion independently and neither disputed it, which is why it is pre-authorised here rather than routed.

    (b) **Input pins: adding an explicit `mask_refine_mode` or `inpainting_enabled` to a test's INPUT, changing no assertion, no test name and no expected value.** This is the mechanism item 12 relies on. Known sites, to be re-derived rather than inherited: `crates/pc-detect/tests/d7_run.rs`'s `recorded_input`, `crates/pc-detect/tests/common/mod.rs`'s input helper, `crates/pc-pipeline/tests/l6_inpaint_pipeline.rs`'s strip helper, and `crates/pc-pipeline/tests/common/mod.rs`'s `options`.

    (c) **What is NOT pre-authorised.** Any change to an assertion, a test name, or an expected value that is not one of (a)'s six sites. In particular, a test that merely goes red under the new defaults is not thereby licensed to be edited: the first question is whether it should be pinned under (b) instead, and if neither class fits, it goes to both architects under cookbook rule 8's second exit.

    (d) **Three new tests are required, and they are additions rather than amendments.** One that a single profile setting `mask_refine_mode = "simple"` and `inpainting_enabled = false` loads, validates, and differs from `Profile::default()` in exactly those two fields; one that the shipped default document's mode line does not carry a comment contradicting the value beside it; and one that `--skip-inpaint` turns off inpainting that is on **by default**, with a premise assertion that the default really is on so the test cannot pass vacuously.

14. **Left OPEN deliberately, flagged rather than resolved by the transcription.** §16.37 item 10(f) is headed *"DECIDED, against the alternative reading: `tests/fixtures/recorded/detector/PROVENANCE.json` is NOT extended."* Item 8(a)'s ruling adds a field to that file, citing item 10(f) for the *method* — through the generator's key list rather than by hand. The two readings are: that item 10(f)'s decision is scoped to the IPP field it was about, since its two grounds do not both generalise. Ground 1 is squarely IPP-specific — *"no artifact that file pins is IPP-sensitive"* — and says nothing about any other field. Ground 2 is about hand-adding a field to a generated file at all: the file is *"generator-produced from a fixed key list … so a hand-added IPP field would be dropped silently at the next re-record"*, where the IPP-ness is incidental and the mechanism is field-agnostic. **Ground 2 therefore cuts in favour of what item 8 does here rather than against it**, because item 8 adds the field through the generator's key list and the typed struct, which is precisely the failure mode ground 2 exists to prevent, in which case adding a different field for a different reason needs no marker; or that its heading states a general rule about that file, in which case this entry owes it one. The transcription does not add a marker for it and does not decide it — the same handling §16.39 item 5(g) gave the equivalent question about §16.37, and for the same reason: a transcriber may flag a scope question, and may not settle one. Two further items are open and unchanged by this entry: the `Simple`-as-a-v1-addition question from item 2(b), and the calibration document from item 9(c).

15. **Task sequencing, with the `heavy`/`simple` classification the batching rule requires.** **D1** (**heavy**): the A5 gate of item 9, the three recorder pins and the two provenance fixes of item 8, the mode-derived report strings, and the two tripwire tests. D1 lands **before** any default moves, so it is green both before and after the flip and each commit stands on its own. **D2** (**simple**, after D1): the two `Default` impls, the shipped TOML, item 13(a)'s six value corrections, item 13(b)'s input pins, and item 13(d)'s three new tests. **D3** (**heavy**): item 11(b)'s `Requirement::Required` promotion and its partition test. **D4** (**simple**, last, after D1–D3 so line numbers have settled): the claim-site sweep — `crates/pc-testkit/tests/a4d_claim_sites.rs`'s rows and counts with every line number re-derived by grep rather than incremented, `README.md` reframed around opting out, `docs/DETECTOR_ORACLE.md`, `crates/pc-models/src/lib.rs`'s prose, and the code comments across `pc-detect`, `pc-inpaint`, `pc-imageops` and `pc-cli` that cite the old defaults. The implementer runs the full verification bar and does not review the result; §16.13 item 4 applies unchanged.

## 16.47 GPU-2: real CUDA registration for the text detector, a per-stage refusal for OCR and LaMa, and the mode-bench mechanism-sentence correction (joint architect + Senior Rust Engineer plan pass, converging on every point actually argued; maintainer decisions on scope and gate, 2026-08-11)

**Provenance.** Two independent Opus planning passes (`architect`, read-only design; `rust-engineer`, co-plan with drafted test code), both against the same brief, both reading the pinned `ort` 2.0.0-rc.12 source and — unusually for a planning pass in this project — both *running* real probes rather than only reading: the architect built this against the maintainer's actual machine state (measured below); the rust-engineer built an isolated scratch crate at the same `ort` pin and ran it with and without `ort/cuda`. Neither plan disagreed with the other on any point both addressed; each found real things the other's plan omitted, verified directly against the repository by the Orchestrator before this entry was written (not trusted from either self-report) — see item 2. Three points were decided by the maintainer directly, not adjudicated: item 1 (keep the single global device switch, do not reverse §16.36 item 1's per-stage question the discussion raised), item 6 (fix the mode-bench collision now, not deferred), and item 9 (add a `cuda` feature test run to this task's verification gate). None of these three needed Fable — they were the maintainer's own product/process calls, the same class as §16.46's LaMa-Required promotion.

   **Non-retrievability disclosure, same class as §16.45's.** Neither plan's full report is a committed, independently-retrievable artifact — both lived in the dispatching session's conversation, and `docs/briefs/` is gitignored. Where this entry says "neither plan disagreed," "each found real things the other's plan omitted," or "converging on every point actually argued" (heading), that is the Orchestrator's own characterization of the two returned reports, not a pointer to a retrievable transcript. Every code and quote claim below that this entry treats as verified was independently re-checked against the repository or the pinned `ort` source directly, not taken on either plan's word; every hardware/toolchain claim (item 2(a)) was independently re-checked by running the actual machine probe (`nvidia-smi`, a directory search for `cudnn64_9.dll`, an isolated `--features cuda` build), which is a third evidence channel neither the repository nor `ort`'s source can substitute for.

1. **DECIDED (maintainer): the single global `[general] device` switch stands. §16.36 item 1 is NOT reversed.** A per-stage device redesign was considered — raised because the current design makes `device = "cuda"` refuse the whole run unless every device-touching stage has a vetted GPU path, which reads as a conflict between "I want the detector fast" and "OCR/LaMa aren't ready." Rejected on the maintainer's own ground: a user who has a GPU generally wants the whole pipeline on it, not a fragmented per-stage config, so the right fix is to give OCR and LaMa their own real, measured CUDA paths (items 10–11 below name these as GPU-3/GPU-4) rather than to fragment the device surface. This task's scope is therefore **exactly** what §16.23 item 5 and §16.36 item 7 already named: **the `cuda` feature and its guards, for the text detector.** OCR's own CUDA execution stays out of scope per §16.36 item 3's binding condition, unchanged; LaMa's stays out of scope by this entry's own decision (items 4, 11) — §16.36 item 3 is scoped to `pc-ocr` throughout and rules nothing about LaMa, which §16.38 item 15(d) instead ties to the same resolver without granting it a CUDA path.

2. **Ground truth this entry corrects or adds to §16.36's record, each independently verified — against the repository, the pinned `ort` source, or (item 2(a) only) a direct machine probe — not assumed from either plan.**

   (a) **This machine has real CUDA hardware, missing one runtime library.** Measured: an NVIDIA RTX 4090 (driver 591.86, CUDA 13.1 as the driver's *maximum* supported version — §16.22 item 5(h)'s cu12-not-cu13 ruling already covers why cu12 is still the right distribution), a CUDA 12.1 toolkit installed, including `cudart64_12.dll`/`cublas64_12.dll`/`cublasLt64_12.dll`/`cufft64_11.dll`/`curand64_10.dll`. Four of these five — `cudart64_12.dll`, `cublasLt64_12.dll`, `cublas64_12.dll`, `cufft64_11.dll` — are exactly `ort`'s Windows `CUDA_DYLIBS` list (`ort-2.0.0-rc.12/src/ep/cuda.rs:400`, verified by reading the pinned source); `curand64_10.dll` is present as part of the toolkit but has no entry on `ort`'s Windows list at all — the Linux list names the differently-spelled `libcurand.so.10` — so it is not required by `ort` for CUDA registration on Windows — but **cuDNN 9 (`cudnn64_9.dll`) absent**, confirmed by a depth-6 search. A `--features cuda` build was actually compiled and run in an isolated scratch crate at the same `ort` pin: `x86_64-pc-windows-msvc` resolves `ort-sys`'s `cu12` distribution (82 MB download, `onnxruntime_providers_cuda.dll` 92.3 MB) with **no downgrade to the `none` distribution**. Real, measured registration failure on this exact machine: `Error loading "...onnxruntime_providers_cuda.dll" which depends on "cudnn64_9.dll" which is missing. (Error 126)`. This is not a synthetic edge case — it is the actual, reachable state of "cuda feature compiled in, GPU present, one runtime library missing," and every test below that claims to exercise a `cuda`-feature build was run against it.

   (b) **`ort::ep::CUDA` and `ConvAlgorithmSearch` are exported unconditionally; only `CUDA::register()`'s body is feature-gated — on `any(feature = "load-dynamic", feature = "cuda")`, not `cuda` alone (`ort-2.0.0-rc.12/src/ep/cuda.rs:369`).** This holds as "gated on `cuda`" for this workspace specifically, since the root `Cargo.toml`'s `ort` features (`ndarray`, `std`, `download-binaries`, `copy-dylibs`, `tls-rustls`) do not include `load-dynamic`. Verified by reading `ort` 2.0.0-rc.12's source and confirmed by execution: without `ort/cuda`, `CUDA::register` is compiled to `Err(RegisterError::MissingFeature)`, deterministically, with no dependency on hardware, driver, or cuDNN. **Consequence, load-bearing for item 7 below: the Heuristic-pin and error-on-failure assertions run in the default, no-`cuda`-feature tier, on a machine with no GPU at all.** `ort` sets no `cudnn_conv_algo_search` option key by default (measured: `CUDA::default()`'s `Debug` has empty options); ONNX Runtime's own default is then `EXHAUSTIVE`. `CUDA::is_available()` enumerates providers compiled into the *linked runtime* and needs no GPU, driver, or cuDNN — measured `Ok(false)` without `ort/cuda`, `Ok(true)` with it, on this GPU-having-but-cuDNN-lacking machine — making §16.22 item 5(d)'s downgrade guard runnable without real end-to-end CUDA execution.

   (c) **`pc_core::device` already ships more than §16.36 item 2's quoted API sketch shows.** Verified directly against `crates/pc-core/src/device.rs`: a `ProviderRequest` enum (`Cuda { conv_algorithm_search: ConvAlgorithmSearch }`) and `DevicePolicy::provider_requests(&self) -> &[ProviderRequest]` already exist, already frozen by `crates/pc-core/tests/device_policy.rs`. This is GPU-1's already-built, model-agnostic bridge from a resolved policy to what an `ort` boundary needs to register — this task consumes it, not reinvents it.

   (d) **A frozen manifest test constrains how the `cuda` feature may be forwarded.** `crates/pc-detect/tests/perf2_feature_containment.rs` pins `pc-cli`'s direct `pc-detect` dependency features to exactly `["testkit"]`. `cuda` must be declared and forwarded through each crate's own `[features]` table (`pc-detect`'s `cuda` implying `onnx`, `ort/cuda`, `pc-core/cuda`; `pc-cli`'s implying `onnx`, `pc-detect/cuda`), never appended to a dependency line, or this frozen test breaks for a trivially avoidable reason.

   (e) **`crates/pc-inpaint/tests/l5_session.rs`'s `Ok(_)` branch (lines ~124–134) assumes a build that CAN resolve CUDA falls through to the model-file pre-flight check.** Item 4 below's per-stage refusal for LaMa changes this branch's outcome; this test needs re-ratifying alongside item 4's implementation, not silently left red.

3. **The `ort` boundary lives in `pc_detect::onnx`, converged independently by both plans.** Neither plan disagreed on this — each proposed it without the other's draft in hand, and each explicitly flagged the alternative (a shared `pc-core`-adjacent home, or a new leaf crate) as something they would *not* fight for. Grounds, from both: §16.36 item 2 already ruled the resolver lives in `pc_core::device`, **pure, with no `ort` types and no EP registration** — an `ort`-gated module there would need its own reversal of that ruling. §16.22 item 5(f) forbids a second policy surface — building a shared `ort`-facing home for two stages that are *ratified not to have a CUDA path yet* (item 4) is speculative generality inviting exactly the extension §16.36 item 3 forbids. If a future ratification grants OCR or LaMa their own CUDA path (items 10–11), that task decides the shared home then, with its own measurements in hand — not this one.

   API, additive to `pc_detect::onnx` (both `#[cfg(feature = "onnx")]`):
   ```rust
   pub fn execution_provider_dispatches(policy: &DevicePolicy) -> Vec<ort::ep::ExecutionProviderDispatch>;
   pub fn apply_device_policy(builder: SessionBuilder, policy: &DevicePolicy)
       -> Result<SessionBuilder, StageError>;
   ```
   `execution_provider_dispatches` reads `policy.provider_requests()` (item 2(c)) and maps each `ProviderRequest::Cuda { conv_algorithm_search }` to an `ort::ep::CUDA` configured with that search mode and `.error_on_failure()` — countermanding `ort`'s own default (`ExecutionProviderDispatch::new` sets `error_on_failure: false`, documented as "silently fail and fall back to … the CPU provider," which §16.22 item 5(c) calls "the single most important line in the feature"). `apply_device_policy` is a no-op returning the builder unchanged when `provider_requests()` is empty — a CPU policy remains structurally "no explicit registration" (§16.36 item 2), which is what keeps §16.32's bit-exact recorded floats untouched. It must be called inside `build_session`, before `commit_from_file`, with `&DevicePolicy` threaded through from `OnnxProvider::initialize_detector`, which already resolves one at `crates/pc-cli/src/detector.rs:101` and today discards it (`resolve(...).map_err(...)?;` with the `Ok` value dropped).

4. **New `pc_core::device` surface: a typed, stage-scoped refusal — RULED (Orchestrator, not escalated, since this is a representation choice both plans converged on in substance and differed on only in encoding).**

   **SUPERSEDED IN PART by §16.48 item 2 — this note covers the whole item, not only the quoted message below.** §16.48's real, measured OCR CUDA path deleted `Stage::Ocr` outright: the `pub enum Stage { Ocr, Inpaint }` shape shown just below is stale (it is `{ Inpaint }` only today), the per-stage `match` that selected each stage's citation/flag/section (described a few paragraphs down as "keying the message on `Stage`") was replaced by a single fixed tuple since only one stage remains, and the "Wired at both remaining seams... (OCR, `Stage::Ocr`)" sentence below no longer describes a live call site — `crates/pc-ocr/src/onnx.rs` and `crates/pc-cli/src/ocr.rs` no longer call `ensure_stage_supports` at all. What survives unchanged: `ensure_stage_supports`'s two-call-ordering pattern (`resolve` then the stage check) for the one remaining stage, and `Stage::as_str()` as the wire-spelling source. Both plans independently proposed the same shape — a new `DeviceRefusal` variant naming the stage that has no ratified CUDA path — differing only in whether "stage" is a typed enum (architect) or a `&'static str` (rust-engineer, offered without arguing against the enum). Ruled: the typed enum, for consistency with `pc_core::device`'s existing `Device`-enum style and because a closed set of two stages needs no string:
   ```rust
   pub enum Stage { Ocr, Inpaint }
   pub enum DeviceRefusal {
       NotCompiledIn { requested: Device },                          // unchanged, GPU-1
       NoRatifiedStagePath { requested: Device, stage: Stage },      // NEW, additive
   }
   pub fn ensure_stage_supports(policy: &DevicePolicy, stage: Stage) -> Result<(), DeviceRefusal>;
   ```
   Verified additive against the frozen `device_policy.rs`: of its two frozen refusal tests, neither matches `DeviceRefusal` exhaustively — the message test (`the_not_compiled_in_refusal_message_is_frozen`) and the variant test (`a_cuda_request_on_a_cpu_only_build_is_refused_naming_the_device`) both construct `NotCompiledIn` explicitly, so a new variant is additive to both.

   **Ordering at each of the two seams is load-bearing, and is what lets this task land with zero edits to any G1-C assertion:** `resolve(device, DeviceSupport::compiled())?` first, `ensure_stage_supports(&policy, stage)?` second. In a non-`cuda` build the first call already refuses with the frozen `NotCompiledIn` message, and **five of the six** existing G1-C tests stay green because their gate matches the build: `crates/pc-cli/src/detector/g1_c_tests.rs:11` carries `#[cfg(not(feature = "cuda"))]` (the module itself is already gated `#[cfg(all(test, feature = "onnx"))]` at `crates/pc-cli/src/detector.rs:373`, which supplies the `onnx` half); `crates/pc-cli/tests/g1_c_device_refusal.rs:89,123,148,185` each carry the full `#[cfg(all(feature = "onnx", not(feature = "cuda")))]` directly. Both files carry a file-level `#![allow(unexpected_cfgs)]`, pre-written by GPU-1 for exactly this task per §16.36 item 7.

   **The sixth G1-C test carries no `cuda` cfg at all, and this task must not assume its outcome.** `crates/pc-cli/tests/g1_c_device_refusal.rs:236`'s `replay_and_mock_with_ocr_disabled_accept_cuda_without_creating_a_session` runs in every tier, including the newly-mandated `cuda` tier (item 9). Its own doc comment states its premise: *"a configured device is inert when neither selected backend creates an ONNX session"* — with `ocr_enabled = false` and a replay/mock detector, no seam this task touches (item 4's OCR/LaMa refusals, item 3's detector registration) should ever be reached, so this test is expected to stay green under a real `cuda` build for the same reason it is green today. **Expected, not assumed:** whoever implements G2-C/G2-D must run this specific test under `--features xtask/cuda` (transitively enabling `pc-cli/cuda`) and confirm it, rather than inferring it from this paragraph.

   In a `cuda` build the first call succeeds and the second refuses — with a message that does **not** tell the user to rebuild with a flag they already used, which reusing `NotCompiledIn` for this case would do and which is exactly the false-message defect both plans flagged as the reason a second refusal shape is required rather than optional.

   **Message text, offered by the rust-engineer's plan, adopted here** (config-key paths verified against `crates/pc-config/src/default_profile.toml`: `ocr_enabled` under `[preprocessor]`, `inpainting_enabled` under `[inpainter]`, `device` under `[general]`). **The citation is per-stage, not shared** — per B3/item 1's correction, §16.36 item 3 rules on `pc-ocr` only:
   > the cuda execution provider is compiled into this build, but no CUDA path is ratified for the {stage} stage ({§16.36 item 3 for Ocr, §16.38 item 15(d) for Inpaint} / §16.47 item 1: CUDA has only been measured for the text detector). Set {the stage's enabled flag} = false to run the detector on CUDA, or set device = "cpu" to run every stage on the CPU execution provider.

   **SUPERSEDED IN PART by §16.48 item 2 — read the note at this item's own opening, above, for the full scope; this quote specifically is stale in three places, not one.** The Ocr branch of this citation no longer exists as a constructible case; the Inpaint half's "only been measured for the text detector" parenthetical went stale the moment OCR was measured too, so the citation moved AND the clause now reads "has now been measured for the text detector and OCR"; and the flag-advice sentence changed from "run the detector on CUDA" to "run the detector **and OCR** on CUDA," for the same reason. §16.48 item 2 quotes the exact replacement message verbatim.

   Wired at both remaining seams: `crates/pc-ocr/src/onnx.rs:79` and `crates/pc-cli/src/ocr.rs:35` (OCR, `Stage::Ocr`); `crates/pc-inpaint/src/onnx.rs:273` (LaMa, `Stage::Inpaint`) — and item 2(e)'s `l5_session.rs` branch is re-ratified in the same motion: a build that can resolve CUDA now refuses at the stage seam rather than falling through to the model pre-flight, and the test's assertion changes accordingly, not its intent.

5. **`pc-cli` binds the policy it currently discards.** `crates/pc-cli/src/detector.rs:101`'s `resolve(...)` call keeps its `Ok(DevicePolicy)` and threads it into `OnnxProvider::from_path_with_config_and_policy` (new, delegating from the existing `from_path_with_config`, which now passes `&DevicePolicy::cpu()` — no other call site moves). `crates/pc-cli/Cargo.toml` gains `cuda = ["onnx", "pc-detect/cuda"]`. `pc-ocr` and `pc-inpaint` get **no** `cuda` feature of their own — they have no CUDA path (item 1), and giving them a feature flag would advertise a capability item 4 explicitly refuses.

6. **DECIDED (maintainer): the mode-bench collision is fixed as part of this task, not deferred.** This task's own change (item 3: the detector's constructor now takes a `&DevicePolicy` and performs real registration) falsifies an already-shipped, verbatim-quoted ratified sentence and its **four** derived artifacts — not two, corrected from an earlier draft of this item that under-enumerated its own readers (cookbook rule 14). Quoted so the correction is checkable against the original (§16.43 item 6, Fable's graft, quoted there "in full because it is itself binding, not merely explanatory"):

   > "the detector's per-stage row must state that its constructor takes no device argument at all — device reaches the detector path only as an up-front refusal, never as a registration — and the inpainter's row must state that its construction route (`from_path_for_device`) re-resolves the same requested device through the same resolver. …"

   The **inpainter** half is untouched by this task (LaMa still re-resolves the same requested device through the same resolver, now also through item 4's stage-scoped refusal — the mechanism description is unaffected) and needs no correction. The **detector** half is false the moment item 3 lands: the detector's constructor now takes a `&DevicePolicy` argument, and device reaches it as a real registration, not only as a refusal.

   **SUPERSEDES: §16.43 item 6**

   Scope, stated so the marker is not read wider than its evidence: this claims only §16.43 item 6's **detector**-mechanism sentence (the graft's first half, quoted above). The inpainter sentence and every other part of item 6 — the one-process-wide-device-statement ruling, the per-stage-row shape, the "does not amend §16.36" scope clause — are unaffected and stand.

   **All four shipped occurrences, enumerated rather than summarised** (each carries the shipped text "its **session** constructor …" — this entry's own quote above drops "session", corrected here): `xtask/src/mode_bench.rs:51` (`DETECTOR_DEVICE_MECHANISM`, the named constant); `xtask/src/mode_bench.rs:2282` (a **hard-coded duplicate literal**, not a reference to the constant, inside the frozen unit test `the_detector_and_inpainter_rows_disclose_their_two_device_mechanisms_verbatim`); `xtask/tests/mode_bench_cli.rs:93` (a second hard-coded literal, inside an integration test that carries **no `cuda` cfg gate** and runs in every tier); `docs/MODE_COMPARISON.md:30` (the committed generated report). Replacement sentence, for all four: *"its session constructor takes the resolved device policy directly and attempts real registration when a provider is requested — refusal now happens only when the stage has no ratified path for the requested device (§16.47 item 4), not as a substitute for registration."* **Consequence to state plainly:** correcting only the named constant, as an earlier draft of this item scoped it, would leave the other three artifacts asserting the just-superseded sentence — two of them (`mode_bench.rs:2282`, `mode_bench_cli.rs:93`) would go red in the **default** tier the moment item 3's real registration lands, not the `cuda` tier, since neither is `cuda`-gated; the fourth is a committed document that must be regenerated, per this project's standing practice of not leaving a generated report standing on numbers/text already demonstrated wrong.

   **Corrected fix, replacing an earlier draft of this item that named a mechanism `xtask` does not have.** `xtask`'s `mode-bench` command (`run`, beginning at `xtask/src/mode_bench.rs:1124`) resolves a `DevicePolicy` at `:1128`, but that value feeds only `render_document`'s disclosure text — nothing else reads it, and the frozen `cuda_is_a_hard_refusal_...` test drives `--replay`, whose whole point (like §16.44's already-ratified LaMa-acquisition rule) is that no real model is ever built. So a `cuda`-feature build's `--replay --device cuda` **cannot** "attempt real registration and fail on cuDNN" — there is no session for it to register into. The frozen test's own text already scopes it correctly ("a build with no cuda feature"); the defect is in what a `cuda`-feature build must do instead, and in how that tier is reached at all:

   - `xtask` gains its own `cuda = ["pc-cli/cuda"]` feature (unchanged from the earlier draft) — needed because `#[cfg(feature = "cuda")]` inside `xtask`'s own test files checks **`xtask`'s** feature, not `pc-core`'s, regardless of what Cargo's unification does to `pc_core::device::DeviceSupport::compiled()` process-wide.
   - Item 9's gate command selects `xtask/cuda` directly, not `pc-cli/cuda` — this is what actually reaches `xtask`'s own `#[cfg(feature = "cuda")]` code, and it still carries `pc-cli/cuda → pc-detect/cuda → pc-core/cuda` down the same dependency chain, so nothing item 7 needs is lost.
   - The frozen test gains `#[cfg(not(feature = "cuda"))]` and stays otherwise unedited — correct for the default tier, where `compiled()` is still `CPU_ONLY` and the hard refusal still fires exactly as today.
   - Its replacement, under `#[cfg(feature = "cuda")]`, asserts what `--replay --device cuda` **actually does** once CUDA is resolvable: **succeeds and writes a report**, exactly like `--device cpu` does today (mirroring the existing control test `cpu_is_accepted_so_the_cuda_refusal_test_is_refusing_cuda_and_not_the_subcommand`, `xtask/tests/mode_bench_cli.rs:164-185`) — because `--replay` never builds a real session for any device, so a resolvable-but-unexercised CUDA policy changes only the disclosed device text, never whether the run succeeds. Do **not** assert a registration-failure string anywhere in this test: that string is a property of the running machine's CUDA/cuDNN installation, not of this codebase, the same caution §16.36 item 7's downgrade-guard discussion already raised. The real "does a `cuda` build actually attempt registration and fail loudly on this machine's missing cuDNN 9" question is answered by item 8's manual verification against a real model, not by `mode-bench --replay`.

7. **Minimal test list.** Both plans converged that the detector-only scope needs few genuinely new tests; the maintainer's standing test-minimalism instruction is binding here. Six rows, eight tests, each naming what concrete wrong value turns it red — plus item 6's own ninth, described there rather than duplicated here. Two rows name more than one test each, for different reasons: the manifest-coherence row names two independent assertions (no crate's `default` reaches `cuda`; the feature correctly implies `onnx`/`ort/cuda`/`pc-core/cuda`), and the downgrade-guard row names the guard plus its anti-vacuity control (a build with no `cuda` feature must report the provider absent, or the guard could pass on a downgraded build for the wrong reason):

   | Test | Tier | Requirement | Falsifier |
   |---|---|---|---|
   | `compiled_support_includes_cuda_exactly_when_the_cuda_feature_is_on` (`pc-core`, new file — the frozen `device_policy.rs` stays untouched) | both cfg arms in one test, runs in every tier | §16.36 item 7 bullet 1 | `compiled()` unconditionally `CPU_ONLY` (the cuda arm fails) or unconditionally `WITH_CUDA` (the default arm fails, in the standard bar) |
   | `no_crate_reaches_the_cuda_feature_from_its_default_feature_set` + `the_cuda_feature_pulls_in_both_the_onnx_backend_and_orts_cuda_linkage` (`pc-detect`, new file, `toml_edit`-based, same pattern as `perf2_feature_containment.rs`) | every tier | §16.22 item 5(a); this task's own manifest-coherence invariant | `cuda` reachable from any crate's `default`; `pc-detect/cuda` missing `onnx`, `ort/cuda`, or `pc-core/cuda` |
   | `a_cuda_policy_pins_heuristic_conv_algorithm_search_on_the_ort_provider` (`pc-detect`) | default tier, no GPU needed (item 2(b)) | §16.22 item 3; §16.36 item 7 bullet 4 | the dispatch missing the `"HEURISTIC"` key `ort` itself only sets when told to, or carrying `EXHAUSTIVE` |
   | `the_cuda_dispatch_carries_error_on_failure_rather_than_orts_silent_default` (`pc-detect`) | default tier, no GPU needed | §16.22 item 5(c) | omitting `.error_on_failure()` |
   | `a_cuda_registration_that_cannot_succeed_is_a_loud_model_error` (`pc-detect`, `#[cfg(not(feature = "cuda"))]`) | default tier | §16.36 item 4 (run-fatal classification) | a registration failure producing anything other than `StageError::Model` |
   | `the_linked_runtime_carries_the_cuda_provider_and_not_the_none_distribution` + its anti-vacuity control `the_downgrade_guards_probe_reports_false_when_no_cuda_distribution_is_linked` (`pc-detect`) | `cuda` tier / default tier respectively | §16.22 item 5(d); §16.36 item 7 bullet 3 | `is_available()` true with no `cuda` feature, or a guard that would also pass on a downgraded build |

   Plus item 4's re-ratification of one existing `l5_session.rs` branch, and item 6's mode-bench test split (one `cfg`-gated, one replaced). **Explicitly rejected, so the omission is a decision, not an oversight:** a `compiled()`-equals-`WITH_CUDA` re-assertion under the feature alone (folded into the first test's premise, where it protects something); re-testing the recording refusal, `DevicePolicy::report()`'s text, or config load/round-trip (all already frozen, all verified unaffected: `xtask/src/device.rs`'s `ensure_recording_request`/`ensure_recording_policy` key on `requested != Device::Cpu`, capability-independent; `xtask/tests/g1_d_resolved_policy.rs` already exercises the resolved-policy arm through an explicit `WITH_CUDA`; `report()` gets no new caller); a session-provider-placement test, which §16.22 item 5(e) already records is unwritable — `ort` rc.12 exposes no such introspection.

8. **A real, disclosed coverage gap: nothing in this task's test suite drives an actual `build_session` call with a non-`cpu` policy end to end, because no `.onnx` fixture is committed to this repository.** (`find . -name "*.onnx"` outside `target/` returns nothing.) §16.36 item 6 already warned about exactly this class of hazard: "a refusal test that never observes a non-default `device` because the plumbing dropped it silently would pass while the feature does nothing." This task's tests close the `ort`-boundary and manifest questions but cannot close this one without a real model file. **Required before this task is treated as done:** a manual `--features cuda` run against the maintainer's real, already-cached detector model (`%LOCALAPPDATA%\panel-ocr\models\comictextdetector.pt.onnx`, verified present and hash-checked in an earlier session) on this machine, with the observed output — the real cuDNN-9-missing registration failure, surfaced as a loud `StageError::Model` and a non-zero exit, not a silent CPU fallback — recorded in `docs/WORKSTATE.md`. The independent reviewer (§16.13 item 4) checks the call site by reading, not by trusting this record.

9. **DECIDED (maintainer): the verification gate for this task adds `cargo test --workspace --features xtask/cuda`.** Selecting `xtask/cuda` rather than `pc-cli/cuda` directly is required by item 6's fix: `xtask`'s own `#[cfg(feature = "cuda")]` test code checks `xtask`'s feature specifically, which Cargo's unification does not turn on merely because `pc-core`'s does; selecting `xtask/cuda` still carries `pc-cli/cuda → pc-detect/cuda → pc-core/cuda` down the same chain, so items 7's `pc-detect`-tier tests are unaffected. **What is actually confirmed today (item 2(a)): an isolated scratch crate at the same `ort` pin, built with plain `ort/cuda`**, resolves the `cu12` distribution (~82 MB download; the separate 92.3 MB figure is the linked `.dll`'s on-disk size after build, not a download size) with no downgrade. **Not yet run: the actual mandated command, `cargo test --workspace --features xtask/cuda`**, since the `xtask`/`pc-cli`/`pc-detect`/`pc-core` `cuda` features this task ratifies do not exist yet — that command is this task's own deliverable, not something confirmed in advance. Worth stating since the chain implies it: selecting `xtask/cuda` transitively turns on the whole `onnx` tier across `pc-cli`/`pc-detect`/`pc-ocr`/`pc-inpaint` for that run, roughly 930 MB of additional `target/` directory expected by extension from the scratch-crate measurement. `cargo clippy --all-features` already compiles the `cuda` branch (asserting nothing); this task's addition is a real test *run*, not merely a compile, and is what makes item 7's `cuda`-tier tests non-decorative. This is additive to the existing four-command bar for this task only — whether it becomes permanent for every future task is not decided here.

10. **Task breakdown and sequencing.**

    | Task | Scope | Class |
    |---|---|---|
    | **G2-A** | `cuda` feature declared on `pc-core`/`pc-detect`/`pc-cli`/`xtask`; `DeviceSupport::compiled()` becomes conditional; the two manifest-containment tests | simple |
    | **G2-B** | `pc_core::device::{Stage, DeviceRefusal::NoRatifiedStagePath, ensure_stage_supports}`; wiring at the OCR and LaMa seams (item 4); the `l5_session.rs` re-ratification | simple, batch with G2-A |
    | **G2-C** | The `ort` boundary (item 3): `execution_provider_dispatches`/`apply_device_policy`; `build_session` and `OnnxProvider` threading; `pc-cli/src/detector.rs` binding the policy it drops; the three `pc-detect` tests of item 7 | heavy, own call |
    | **G2-D** | The downgrade guard (item 7's last row) plus item 8's manual real-model verification, recorded in `docs/WORKSTATE.md` | simple, after G2-C |
    | **G2-E** | The mode-bench fix (item 6): `xtask`'s `cuda` feature, the `#[cfg]` gate on the frozen test, the replacement `cuda`-tier test, and the correction to **all four** shipped occurrences of the device-mechanism sentence (`DETECTOR_DEVICE_MECHANISM`, the frozen test's duplicate literal, the ungated integration test's duplicate literal, `docs/MODE_COMPARISON.md` regenerated) | heavy, own call — every edit here touches a frozen or ratified artifact |

    Order: G2-A + G2-B batched → G2-C → G2-D → G2-E. G2-E last, for the same reason §16.46's D4 was last: until G2-C lands, the mechanism sentence is still true, and correcting it early would put a false statement in the tree in the other direction.

    **Implementation hazard for G2-E, named rather than discovered mid-task — corrected from an earlier draft's over-broad claim.** Gating the frozen `mode_bench_cli.rs` test and the four gated `g1_c_device_refusal.rs` tests behind `#[cfg(not(feature = "cuda"))]` risks orphaning their supporting items under `cargo clippy --workspace --all-targets --all-features -- -D warnings` (the standing bar, which enables `cuda` once it exists), but **measured against the sixth, ungated test (item 4's note above), only one helper is actually orphaned: `assert_exact_cli_refusal`** (used solely by the three gated call sites at lines 171, 205, 225). `cuda_refusal`, `write_cuda_profile`, `write_page`, `run`, and `binary` remain used by `replay_and_mock_with_ocr_disabled_accept_cuda_without_creating_a_session` (line 236, ungated) and are not orphaned. `mode_bench_cli.rs`'s `CUDA_REFUSAL` const's sole use is inside the test being gated, so it **is** orphaned and needs its own `#[cfg]` or removal. Verify the standing clippy bar stays clean under the `cuda` feature before treating G2-E as done — item 4's claim of "zero edits to any G1-C **assertion**" stays literally true, but `assert_exact_cli_refusal` and `CUDA_REFUSAL` need their own `#[cfg]` gates or removal.

11. **Explicitly deferred, named so the goal is not lost: GPU-3 (real, measured CUDA for OCR) and GPU-4 (real, measured CUDA for LaMa).** Each repeats this task's own recipe for its stage — §16.22-style nondeterminism and cross-provider divergence measurement, then real registration — and each, once landed, removes one of item 4's two stage-scoped refusals by replacing it with a real path. Not sequenced relative to the rest of the v1.5 backlog (legacy INI import, PSD, DBNet) by this entry; that placement is decided when this task is done. Per the maintainer's explicit merge policy: the `v1.5` branch does not merge back into `claude/codex-plugin-install-jxirxa` until GPU-2, GPU-3, GPU-4, and the rest of the v1.5 backlog are all done.

## 16.48 GPU-3: real, measured CUDA registration for OCR (manga-ocr encoder+decoder), superseding §16.47 item 4's stale text-detector-only citation (Orchestrator resolution of a sequencing defect found by independent review, real measurement re-run and independently re-verified twice, 2026-08-11)

**Provenance.** GPU-2 (§16.47 item 11) deferred GPU-3/GPU-4 as explicit follow-ups, each to repeat this task's own recipe for its own stage — §16.22-style nondeterminism and cross-provider divergence measurement, then real registration. A joint `architect` + `rust-engineer` (Opus) planning pass against `docs/briefs/gpu3_plan_brief.md` found the machine's cuDNN 9 was missing (blocking real CUDA *execution*, distinct from GPU-2's registration-only gap) and converged on a shared `pc-ort` extraction plus a `Stage::Ocr` design disagreement, tie-broken by `fable-adjudicator` (§16.36-style ruling: delete `Stage::Ocr` outright, per rust-engineer's position, with a mandatory graft neither Opus plan named — `Stage::Inpaint`'s own message shares the same text-detector-only clause and goes stale the moment OCR is measured too). The maintainer decided directly (asked, not adjudicated) to install cuDNN 9 now rather than split and defer the work; the Orchestrator ran the install (`pip install --target=D:\Temp\nvidia-cudnn nvidia-cudnn-cu12`) and confirmed it working with a real before/after comparative run on this machine — the exact CUDA-execution registration failure reproduces with the cuDNN dir off `PATH` and disappears with it on. Implementation (`cmdc`, jobs producing HEAD `881f2d0` for the `pc-ort` extraction, plus an uncommitted `Stage::Ocr` deletion + OCR CUDA registration pass) was independently reviewed (a separate Agent-tool `rust-engineer` spawn, §16.13 item 4) and found a real sequencing defect, not a nitpick: the implementation shipped a message claiming "CUDA has now been measured for the text detector and OCR" before any such measurement had been run, violating §16.36 item 3's and §16.47 item 11's own explicit ordering ("measurement... then real registration"). Root cause recorded honestly: both original Opus plans' task breakdowns put registration before measurement without flagging the conflict, and the Orchestrator carried that ordering into the distilled implementation briefs without independently catching it before dispatch. **This entry's own job is to close that gap** — it exists only once the measurement below was actually run, not before.

**Design note on the measurement's shape, an Orchestrator-level technique call, not re-escalated to Fable** (the same class of substitution as the A5-gate technique call during the earlier defaults-flip mission): two designs were available for the decoder half of the measurement — a naive comparison of two independent free-running decodes' logits, or a **teacher-forced** comparison (replay every prefix the real CPU beam search actually visited through the CUDA decoder, fed the CPU encoder's own output). The teacher-forced design was adopted because it is the only one of the two that isolates the decoder's own numerical divergence from the encoder's and from beam search's own feedback loop — a naive free-running comparison confounds three divergence sources into one number, which cannot support a claim specifically about the decoder.

1. **The measurement was actually run, on this machine, with cuDNN 9 on `PATH`, against the real cached manga-ocr encoder+decoder ONNX weights and the already-committed `demo_bubbles/handwritten_bubble_raw.png` fixture crop — not simulated, not estimated.** Two independent `MangaOcrSessions` were built (`Device::Cpu`, `Device::Cuda`) from the same model files (encoder sha256 `15fa8155fe9bc1a7d25d9bb353debaa4def033d0174e907dbd2dd6d995def85f`, decoder sha256 `ef7765261e9d1cdc34d89356986c2bbc2a082897f753a89605ae80fdfa61f5e8`). Four channels, per `crates/pc-ocr/src/device_divergence.rs`'s comparator library (`compare_tensors`, `first_divergence_index`, `min_top2_margin` — pure functions, `default`-tier, every expected test literal hand-derived per cookbook rules 7/13, not derived by running the function and copying its output) and `xtask/src/ocr_device_compare.rs`'s producer:

   | Channel | What is compared | Values | Bitwise-differing | max abs Δ | mean abs Δ | per-row max-of-maxes | min top-2 margin |
   |---|---|---:|---:|---:|---:|---:|---:|
   | 1 — encoder hidden states, CPU vs CUDA | direct | 151296 | 151288 | 0.009014 | 0.000365 | — | — |
   | 2 — decoder logits, teacher-forced (CPU encoder fed to both) | 69 recorded CPU beam-search prefixes | 423936 | 423837 | 0.004109 | 0.000506 | 0.004109 | 0.108847 |
   | 3 — end-to-end token IDs, CPU free run vs CUDA free run A | — | first divergence `None`; strings equal | — | — | — | — | — |
   | 4 — end-to-end token IDs, CUDA free run A vs CUDA free run B | — | first divergence `None`; strings equal | — | — | — | — | — |

   **Two columns are narrower than they read.** Channel 2's "max abs Δ" and "per-row max-of-maxes" cells are the same number by construction (a max over a partition equals the global max), not two independent measurements — the second column exists to name the *aggregation*, not to corroborate the first. Channel 2's "min top-2 margin" (`0.108847`) is computed over the **CPU** logit rows only, characterizing how close the real CPU decode's own decisions were to flipping — it is not a CPU-vs-CUDA quantity, unlike every other cell in that row.

   Decoded text agrees across all three runs (CPU, CUDA A, CUDA B): `すラこれから目にまわりダスか！？`. Full method, provenance, and environment details (driver 591.86, CUDA 12.1, `ort` 2.0.0-rc.12) are recorded in the committed, non-gating diagnostic `docs/OCR_DEVICE_DIVERGENCE.md`, regenerated by `cargo xtask ocr-device-compare` — **do not hand-edit that file**; it is generated in full at measurement time. **Independently re-run and re-verified twice by the Orchestrator** (not trusted from either the implementer's report — which, for this final measurement task, did not exist at all: the dispatching job returned `status: failed, exit code 8` with an empty output file — or from the file already on disk): once by reading and hand-checking every comparator-library test literal against its own by-hand derivation, and once by re-executing `cargo run --features cuda -p xtask -- ocr-device-compare` fresh against the same cached models and fixture, reproducing byte-identical channel numbers and decoded text.

2. **SUPERSEDES: §16.47 item 4**

   Per §16.22 item 2's CPU-determinism carve-out applying in the negative direction (CUDA is exempt from determinism), none of the numbers above are a pass/fail gate — this is a diagnostic measurement, not a threshold. What it does establish, factually, for citation purposes: **the text detector and OCR (manga-ocr encoder+decoder) both now have a real, measured CUDA execution path on this project.** `crates/pc-core/src/device.rs`'s `NoRatifiedStagePath` message for `Stage::Inpaint` cites this item — not §16.47 item 1, which never said anything about OCR **being measured** (it does discuss OCR: "OCR's own CUDA execution stays out of scope per §16.36 item 3's binding condition" — the narrow, correct claim is about the measurement, not about OCR being mentioned at all) — for its "CUDA has now been measured for the text detector and OCR" clause. The target-side back-pointer for the marker above is recorded at §16.47 item 4's own opening note and its quote, both of which this item's changes affect; that item's own two-call-ordering pattern (`resolve` then the stage check) is unaffected for the one remaining stage, but its per-stage `match` (which used to select each stage's citation/flag/section) is not — it was replaced by a single fixed tuple, since only `Stage::Inpaint` remains.

   The shipped replacement message, quoted verbatim (three changes from §16.47 item 4's original: the citation, the "only"→"now... and OCR" clause, and the flag-advice clause):
   ...
   `Stage::Ocr` is deleted outright (Fable's ruling, above): `Stage` is now `{ Inpaint }` only, `Stage::as_str()` has a single arm, and `ensure_stage_supports`'s `Device::Cuda` arm constructs only `NoRatifiedStagePath { stage: Stage::Inpaint }`. The frozen `crates/pc-core/tests/g2_stage_refusal.rs` changes accordingly, not only its one hard-coded literal: the now-meaningless `the_ocr_refusal_message_is_frozen_to_its_own_citation_and_flag` test is deleted outright, `a_resolvable_cuda_request_refuses_both_stages` is renamed to `..._the_inpaint_stage` with its Ocr-arm assertions removed, and the two remaining tests each lose the assertion/loop iteration that referenced `Stage::Ocr` — five tests become four, all four still exercising real, falsifiable coverage of the one remaining stage. The two OCR call sites that used to check `ensure_stage_supports(Stage::Ocr)` no longer refuse — `crates/pc-ocr/src/onnx.rs` builds a real CUDA session via `pc_ort::apply_device_policy`, threaded to both the encoder and decoder sessions, the same registration boundary GPU-2 built for the detector (§16.47 item 3); `crates/pc-cli/src/ocr.rs` resolves its own policy but deliberately discards it (documented at the call site, not a mirror of the detector's own `pc-cli` seam, which threads its policy per §16.47 item 5) and lets `pc-ocr` re-resolve internally — a disclosed, accepted residual, not an oversight.

3. **GPU-4 (real, measured CUDA for LaMa) remains explicitly deferred, unaffected by this entry.** `Stage::Inpaint` keeps its refusal — the citation for its "no CUDA path is ratified" half stays §16.38 item 15(d), unchanged; only the "measured" parenthetical moved. GPU-4 will repeat this same recipe (measurement, then real registration) and, once landed, is expected to delete `Stage` and `ensure_stage_supports` entirely (nothing would be left to refuse) — that is GPU-4's own decision to make with its own measurements in hand, not decided here.

4. **Frozen-test consequence, stated once more for emphasis, not as new information — item 2 above already discloses the full extent of the change:** `crates/pc-core/tests/g2_stage_refusal.rs`'s hard-coded literal (added by GPU-2's own review, specifically to catch a citation-swap regression — §16.47's review finding B3) is updated to cite `§16.48 item 2` instead of the stale `§16.47 item 1` clause, matching the shipped message exactly. This is not the file's only change; item 2 enumerates the rest (one test deleted, one renamed with assertions removed, two more losing an assertion/iteration each).
