...
**Item 20 is intentionally left unused here.** `UpstreamBoxOutsideFrame` is untranscribed — the note below states landing it "needs its own ruling" — but the closest existing numeric statement about it counts `Divergence` *enum variants*, not §14 register items: §16.27 item 11 describes it as "not among the 19" known-divergence samples, and landing it later "makes it a 20th variant by its own ratification" (that variant-count language, not a §14 reservation, is what this item avoids colliding with). The items below therefore start at 21, and on this branch are numbered 21, 22, 24, 25, 26 and 27 — 23 being the sibling `mask-parity` branch's, per the note further down; after the two branches merge the run is continuous, 21 through 27. (This sentence read "the two items below are therefore numbered 21 and 22" until §16.38 added four more; it was true when written and false the moment a third item landed, which is why the count is now stated as a start-point plus an explicit list rather than a total. The sibling branch hit this same sentence independently and fixed it the same way — a shared-base sentence carrying a running total is stale by construction the moment either branch extends the list, and a total is the one form of it that cannot survive a parallel edit.)
...
same sequencing item 19's own account describes. **This parenthetical deliberately does NOT say
...
2026-08-06 the numbered markers present under `src/` are **1–16, 18, 19, 21, 25 and 26** — so
**17, 22, 24 and 27 have no site comment today**, and nothing here should be read as asserting
anything about any other row. (Re-measured with that exact command after task L2+L3 landed
`DEVIATION(25)` at `crates/pc-models/src/lib.rs`'s new `ModelSpec` constant and `DEVIATION(26)` at
`crates/pc-config/src/round_trip.rs`. The sentence previously read **1–16, 18, 19 and 21** with
**17, 22 and 24–27** outstanding, which was correct when written and false the moment those two
landed — the same staleness this parenthetical's own warning is about, so it is re-measured rather
than re-asserted, and 24 and 27 are named individually instead of as a range because a range is
what made the previous version wrong in one edit.) Two traps in that measurement, both hit while writing this sentence: a marker in a
...
counted above. Rows 24–27 are §16.38's; 25 and 26 landed with task L2+L3 and 24 and 27 still await
L4 and L5; 17's and 22's state is pre-existing,
