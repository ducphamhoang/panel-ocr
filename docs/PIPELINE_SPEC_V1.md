...
[inpainter]                               # v1.5, ratified by §16.38 item 13. Values are
                                          # `config.py:817-824`'s dataclass defaults, which
                                          # §16.38 item 7(b) established are upstream's runtime
                                          # authority -- NOT `media/default.conf`'s, which
                                          # disagrees on two keys and is read by no upstream code.
inpainting_enabled            = false
inpainting_min_std_dev        = 15.0
inpainting_max_mask_radius    = 6         # INERT, in this port and upstream: the gate its own
                                          # comment describes is `inpainting.py:89`'s
                                          # `thickness <= min_inpainting_radius`. Parsed and
                                          # validated, never read by the eligibility filter;
                                          # setting it away from this default emits a one-time
                                          # WARN, which is DEVIATION(26). §16.38 item 7.
...
**N2's crate column became true again at v1.5 — see §16.38 item 20.** The N2 row above says `pc-imageops`, §16.10 item 1 moved the module to `pc-denoise` for v1, and §16.38 item 20's hoist puts it back where this row always said it belonged. This note is therefore a *restoration* notice, not a staleness one, and deliberately carries no supersession marker: there is nothing here to supersede once the code matches the row. **N1's row is untouched** — `nlm` stays in `pc-denoise` per §16.10 item 1, so that row remains inaccurate for v1.5 exactly as it was for v1.
...
panel-ocr models   download [--include-optional] | verify [--include-optional] | path
```
...
**Item 20 is intentionally left unused here.** `UpstreamBoxOutsideFrame` is untranscribed — the note below states landing it "needs its own ruling" — but the closest existing numeric statement about it counts `Divergence` *enum variants*, not §14 register items: §16.27 item 11 describes it as "not among the 19" known-divergence samples, and landing it later "makes it a 20th variant by its own ratification" (that variant-count language, not a §14 reservation, is what this item avoids colliding with). The items below therefore start at 21, and on this branch are numbered 21, 22, 24, 25, 26, 27 and 28 — 23 being the sibling `mask-parity` branch's, per the note further down; after the two branches merge the run is continuous, 21 through 28. (This sentence read "the two items below are therefore numbered 21 and 22" until §16.38 added five more; it was true when written and false the moment a third item landed, which is why the count is now stated as a start-point plus an explicit list rather than a total. The sibling branch hit this same sentence independently and fixed it the same way — a shared-base sentence carrying a running total is stale by construction the moment either branch extends the list, and a total is the one form of it that cannot survive a parallel edit.)
...
**Item 23 is not absent by accident: it belongs to the parallel `mask-parity` branch.** That branch's committed `1b4e12f` registers `DEVIATION(23)` for `get_topk_color`'s histogram tie order (ratified by its own §16.37), and both branches fork from `02b5613`, where §14 ended at `…19, 21, 22`. Item 23 appears on this branch only after the two are merged; §16.38 item 17(d)(ii) records the collision and how it was avoided. This is the same kind of note as item 20's above, with a different cause — 20 is unused by anyone, 23 is used by someone else.

24. **Per-tile 512×512 inference, in place of upstream's single full-page call — ratified by §16.38 items 4-5 (joint architect + Senior Rust Engineer plan pass, 2026-08-06).** Upstream calls its inpainting model once per page, on the full-resolution original, with one page-global mask (`pcleaner/inpainting.py:132`, guarded by `if boxes_to_inpaint:` at `:131`), and tolerates any page size because its artifact is a fully-convolutional TorchScript generator. v1.5's ONNX artifact has its `image`/`mask` H and W axes fixed at a literal 512 — measured two independent ways at §16.38 item 1(b) (raw protobuf decode) and item 1(d) (three off-size inputs rejected by a real `ort` session) — so a tile loop is forced, not chosen, and the 512-context per call is a genuine behaviour difference from upstream's page-wide context. The tiling and stride policy is declared at §16.38 item 5; its visual consequence is unmeasured and §16.38 item 17(b) keeps that open. The implementation site is the tile loop in `crates/pc-inpaint/` and must carry `DEVIATION(24)`.
25. **Manga-finetuned ONNX weights in place of upstream's TorchScript checkpoint — ratified by §16.38 item 6.** Upstream downloads `anime-manga-big-lama.pt` (`pcleaner/model_downloader.py:21-22`, sha256 `479d3afdcb7ed2fd944ed4ebcc39ca45b33491f0f2e43eb1000bd623cfb41823`) and loads it through `simple_lama_inpainting` by setting `LAMA_MODEL` (`pcleaner/inpainting.py:22-24`). v1.5 loads `lama-manga.onnx` from the `mayocream/koharu` repository at revision `15439cba09df388c51de6e47c6020bc31edab41f` (207,482,644 bytes, sha256 `50a1abae0d73bd46d08eae36c8590cd59ad09029494c9698702b050ef00b0100`), because this project has no TorchScript loader and `.pt`/torch loading is v2 per §16's out-of-scope list. Provenance chain: the sibling `mayocream/lama-manga` repository's `config.json` declares `source_checkpoint` as upstream's exact `.pt` URL, character for character. That is a republisher's claim verified as a string match and **not** as numerical equivalence at any tolerance; §16.38 item 6(a) states the limit rather than leaving it implied. The weights are fetched at run time and never vendored, which is the basis on which the maintainer accepted the artifact's AGPL-3.0 licence. The implementation site is the new `pc_models::ModelSpec` constant and must carry `DEVIATION(25)`.
26. **A one-time `WARN` for the inert `inpainting_max_mask_radius` key — ratified by §16.38 item 7.** Upstream declares, INI-exports, INI-imports and clamps the key (`pcleaner/config.py:819`, `:861`, `:910`, `:920-921`) while its inpainting algorithm never reads it: `pcleaner/inpainting.py` references it zero times across the whole file, and the eligibility gate its own config comment describes (*"The maximum radius of a mask to perform inpainting on"*) is implemented at `inpainting.py:89` against a different key, `min_inpainting_radius`. v1.5 keeps the key equally inert — that half is exact parity and is not the deviation — but adds a `WARN` upstream does not have, emitted once when a loaded profile sets the key away from its default, naming `min_inpainting_radius` as the key that actually gates. Ground: §14 item 7's rule that an opt-in setting must not silently behave differently. The implementation site is `crates/pc-config/src/round_trip.rs` and must carry `DEVIATION(26)`.
27. **Lazy construct-and-latch for the inpainting session — ratified by §16.38 item 8.** Upstream constructs its `InpaintingModel` once, conditional on `inpainting_enabled`, before its per-page loop (`pcleaner/main.py:390-392` sets the skip flag, `:625-626` constructs, the loop is at `:642`); v1.5 defers construction to the first page that has an eligible region and latches that single attempt's success or rendered refusal for the run. Item 16 above records the same placement deviation for the **detector** and is deliberately **not** widened to cover this one, because widening a claim past the subject it names is a separate argued step. Ground: a run with `inpainting_enabled = true` and no eligible region on any page must not pay a 207 MB download plus a one-time session build measured at ~6.3 s (§16.38 item 1(f)) in order to inpaint nothing, and upstream's own `if boxes_to_inpaint:` guard at `pcleaner/inpainting.py:131` shows the empty case is expected rather than pathological. The implementation site is the inpainter provider in `crates/pc-cli/` and must carry `DEVIATION(27)`.

28. **The inpainting fill mask is binarised on ALPHA, not on `convert("1")`'s luma — ratified by §16.38 item 21 (Fable tie-break, 2026-08-07).** Upstream binarises the RGBA `_combined_mask.png` with `mask_image.convert("1")` (`pcleaner/inpainting.py:63`). The input is RGBA and not greyscale — upstream builds it through `combine_best_masks` / `convert_mask_to_rgba`, carrying both fill colour and opacity — and PIL's `convert("1")` routes through `L`, the luma of the RGB channels, **discarding alpha**: a black opaque fill `(0,0,0,255)` reads as *uncovered*, so on the `black_bubble` demo page every poorly-fitted region silently gets nothing inpainted (`black_bubble` is a demo fixture/page, not a profile — the established wording, at item 5 above, is "the `black_bubble` denoise output"; §15.6 decides the alpha-vs-luma question but does not use that phrase). `convert("1")` additionally **dithers** — PIL's documented default for mode `"1"` is Floyd-Steinberg error diffusion — so on the intermediate luma values a real anti-aliased or coloured fill produces, upstream's output is a stippled checkerboard rather than a clean bilevel cut. That mechanism is why "replicate upstream verbatim, bug included" is not an option at any reasonable cost: it would mean porting error diffusion, not changing a threshold. v1.5 uses `alpha > 0` instead. Ground, quoted verbatim from item 5 above because that is the principle and not the claim being reused: *"A mask's 'is this pixel covered' signal must not depend on the brightness of its fill color"*. **This is a DELIBERATELY SEPARATE entry, not a widening of item 5.** Item 5 names `denoiser.py:62`'s path into `grow_mask` and its `mask.convert("L")`; this is a different upstream call in a different file feeding a different stage, with a mechanism (the dithering) item 5's text never describes — the same reason item 27 above declined to widen item 16 rather than absorbing the inpainter's placement into it, per §16.38 item 8(e). Item 5's scope is unchanged by this entry. **Note also, so a later reader does not "unify" it away:** this stage binarises two masks two different ways on purpose — `alpha > 0` for the RGBA combined mask, and `BinaryMask::from_gray_threshold(raw_mask, PIL_BINARY_THRESHOLD)` (strict `> 127`) for the greyscale `_raw_mask.png`, which has no alpha and is already strictly bilevel, so dithering is a no-op there and a luma threshold is correct. §16.38 item 21(d) states this at length. The implementation site is `combined_fill_binary` in `crates/pc-inpaint/src/fill.rs` and must carry `DEVIATION(28)`.
...
once that plan lands (items 16, 19, 21, 22 and 24–27 register ahead of their site comment landing, the
same sequencing item 19's own account describes; **item 28 is excluded from that list deliberately,
not by omission** — its register entry and its site comment land in the same change, so it never had a
register-ahead gap for this list to describe. **This parenthetical deliberately does NOT say
which of them have since acquired one**, because that is a fact about the tree rather than about
this list, and a list of it has gone stale in this document four times. The check is
`grep -rhno "DEVIATION([0-9]*)" --include=*.rs crates/*/src/ xtask/src/`, and measured
2026-08-07 the numbered markers present under `src/` are **1–16, 18, 19, 21, 24, 25, 26, 27 and
28** — so of the rows this branch registers, **17 and 22 have no marker under `src/` at all**, and
nothing here should be read as asserting anything about any other row. (Items **20** and **23** are
absent from that list by design, not by omission: 20 is unused by anyone per the note above, and 23
is the sibling `mask-parity` branch's, so neither is owed a site comment here. **Note also that
`mask-parity` minted its own, unrelated item 24** — an Otsu tie rule, in its commit `70a617c`, after
this branch had already taken 24 for the tile loop — which made 24 a cross-branch collision rather
than a number this branch held cleanly. **That collision is resolved: `mask-parity`'s entry is
renumbered to 29, and this branch keeps 24 unchanged**, on the timestamp precedence §16.38 item
17(d)(ii) sets out; that item also records that the sibling-side renumber was still in flight and
not yet observable from this worktree when this was written, so the resolution is a decision to rely
on rather than a state measured on that branch. The `24` in the measured set above is *this*
branch's tile-loop marker and says nothing about the sibling's.) Re-measured with
that exact command after task L4 landed `DEVIATION(24)`, `DEVIATION(28)` and a reference to
`DEVIATION(27)` under `crates/pc-inpaint/src/`. The sentence previously read **1–16, 18, 19, 21, 25
and 26** with **17, 22, 24 and 27** outstanding, which was correct when written and false the moment
L4 landed — the same staleness this parenthetical's own warning is about, so it is re-measured
rather than re-asserted, and the present set is spelled out past 21 instead of as a range because a
range is what made an earlier version wrong in one edit. **Re-measured a second time after task L5,
and the set is byte-for-byte unchanged — which is not the same as L5 having changed nothing.** L5
raised `DEVIATION(27)`'s mentions under `src/` from one to **five** — it added four
(`crates/pc-cli/src/inpainter.rs:4`, `:6`, `:90` and `crates/pc-inpaint/src/lib.rs:30`) and moved
the one that already existed from `lib.rs:73` to `lib.rs:81` — but introduced no new *number*,
so a set-of-numbers
measurement is exactly the measurement that cannot see the change L5 made; what changed is *which
kind* of marker backs 27, stated in trap three below. 17, 20, 22 and 23 remain the numbers absent
from the set, and 29 — the number §16.38 item 17(d)(ii) rules `mask-parity`'s Otsu tie rule moves
to — is correctly absent here, because that is a marker on the sibling branch and this measurement
is of this worktree only. **Three** traps in that measurement, the
first two hit while writing this sentence and the third while re-measuring it after L4: a marker in a
`tests/` file is NOT a site comment, which is what makes 22 look landed to a repo-wide grep (its
only two hits are `crates/pc-config/tests/defaults.rs` and `crates/pc-core/tests/device_policy.rs`,
while item 22's declared site `crates/pc-core/src/device.rs` carries no marker at all); and the
three qualified `DEVIATION(§16.x item N)` markers that do exist are a different form and are not
counted above; and — the third trap, which is trap one's shape with a different container — **a
marker naming row `n` is not necessarily a marker AT row `n`'s declared site.** The trap is kept
here as a **method**, not as a live instance, because the instance that produced it has since
closed. When this sentence was written after L4, `DEVIATION(27)`'s only hit under `src/` was
`crates/pc-inpaint/src/lib.rs:73`, a forward cross-reference in prose (*"`pc-cli`, lazily and
latched (item 8, `DEVIATION(27)`)"*), while item 27's declared site — the inpainter provider in
`crates/pc-cli/` — carried no marker, so 27 appeared in the measured set above without its site
comment having landed. **That is no longer so: 27's site comment landed with task L5**, as
`// DEVIATION(27):` at `crates/pc-cli/src/inpainter.rs:90`, with the module header at `:4-6`
additionally quoting item 27's site sentence verbatim — so 27's presence in the measured set is now
backed by a marker at its declared site and not only by a cross-reference. There are now **two**
prose cross-references under `crates/pc-inpaint/src/`, and they have different histories — stated
apart rather than as one clause, because calling both "moved" contradicts the "added four" list
twelve lines above, which already counts one of them as new: `lib.rs:81` is the pre-L5 `lib.rs:73`
**moved** (byte-identical, and the line quoted above), while `lib.rs:30` is **new text L5 added**
and moved from nowhere. They are why the trap's *shape* still
matters even though its example closed, since a repo-wide count of `DEVIATION(27)` cannot tell a
site comment from the four other mentions. **Trap three now has no live instance in the measured
set**, and that is stated rather than left implied: no number in `1–16, 18, 19, 21, 24, 25, 26, 27,
28` is present there on a cross-reference alone. Row 22 is deliberately **not** offered as trap
three's replacement example — its markers sit in `crates/pc-config/tests/defaults.rs:78` and
`crates/pc-core/tests/device_policy.rs:120` while its declared site
`crates/pc-core/src/device.rs` carries none, which is trap **one**, and 22 does not appear in the
`src/` set at all, so trap three's specific hazard (a number counted as landed on the strength of a
cross-reference) is not what row 22 shows.
Rows 24–28 are §16.38's; 25 and 26 landed with task L2+L3,
**24 and 28 landed with task L4** (24's site comment is the tile loop in
`crates/pc-inpaint/src/lib.rs`, inside `inpaint_page`; 28's is `combined_fill_binary` in
`crates/pc-inpaint/src/fill.rs`), and **27 landed with task L5** at the site named just above;
17's and 22's state is pre-existing,
is stated here only because this sentence's enumeration would otherwise imply it away, and is NOT
fixed by §16.38 — note in particular that item 17's own text asserts its site comment exists; what this note reverts is a claim of *already
...
25. **The L6 pass's remaining decision points, recorded at the status each actually has.** Four clauses, four different statuses, and they are written separately so a reader does not average them: (a) is **OPEN and deferred**, (b) is a **finding** about existing text and not a defect that was fixed, (c) is **RATIFIED**, (d) is **OPEN** and was not part of the planning pass at all.

    (a) **`L6-DP2` — whether `Step::Inpaint` runs per strip segment or once on the stitched strip: OPEN, DEFERRED, and NOT drafted this round.** Measured rather than assumed: `crates/pc-pipeline/src/single.rs` runs stages 1-4 per segment (`run_stages`, `:6`) and exports the stitched result **once** (`merged_strip_export`, `:475`), so inpainting could sit at either end and both are coherent. The lean recorded here is **include it per segment**, on the ground that segments are what the cache and the checkpoint are keyed on and every earlier stage already works that way — but that is a lean, not a ruling: **no test was drafted for it, so nothing in L6-1 through L6-5 pins strip behaviour either way.** Deferred to a later task unless the Orchestrator directs otherwise. Until it is ruled, no clause anywhere may claim that inpainting's long-strip behaviour is defined.
    ...
    **The consequence of (c), stated rather than left to be discovered.** `Step::Inpaint` becomes the **first** `Step` variant with no `impl pc_core::Stage` — measured, not assumed: `grep -rn "impl pc_core::Stage for" crates/*/src/` returns exactly five hits today, one for each of `Detect`, `Preprocess`, `Mask`, `Denoise`, `Export`. So §3's rule — *"each stage crate exposes one pure function"* (`docs/PIPELINE_SPEC_V1.md:321`, quoted verbatim) — acquires one stage whose function is not routed through the trait, and any future code that enumerates `Stage` impls in order to enumerate stages will silently miss inpainting. Nothing in this repo gates that; it is disclosed here because the alternative was to let a later reader find it.
    ...
    (d) **`L6-DP5` — where `InpainterProvider` lives once the pipeline consumes it: OPEN, found while writing this transcription rather than during planning, and recorded at that status.** The trait is in `crates/pc-cli/src/inpainter.rs:42` (L5), and `PipelineCtx` (`crates/pc-pipeline/src/ctx.rs:47-50`) must carry it for L6-4 — but `pc-pipeline` **cannot** depend on `pc-cli`; the edge runs the other way (`crates/pc-cli/Cargo.toml:28`). Two shapes are available. **(i)** Move the trait alone into `pc_pipeline::ctx` beside `DetectorProvider` and re-export it from `pc-cli`, leaving `UnavailableInpainterProvider` and `OnnxInpainterProvider` where they are; that move **does add a new non-dev edge**, and an earlier draft of this clause claimed the opposite — corrected here rather than left standing. `pc-inpaint` is today only a **dev**-dependency of `pc-pipeline`: `crates/pc-pipeline/Cargo.toml:47` sits under the `[dev-dependencies]` table that opens at `:33`, and that file's own comment at `:45-46` says so verbatim — *"This is NOT L6's wiring: nothing under `src/` references `pc-inpaint`, and `Step::Inpaint` does not exist yet."* Re-measured 2026-08-07: `grep -rn "pc_inpaint" crates/pc-pipeline/src/` returns **0** hits. So putting the trait in `pc_pipeline::ctx` — a `src/` module that names `pc_inpaint::Inpainter`, which is where the returned `Arc<dyn Inpainter>` is defined — requires promoting `pc-inpaint` to a real `[dependencies]` entry, a genuinely new edge in the workspace graph. **That edge is an allowed one, and this correction is not an objection to (i):** the same `Cargo.toml` records at `:15-16` and `:43-44` that *"`pc-pipeline` is the ONE crate allowed to depend on every stage"*, §1 rule 2 forbidding only stage-to-stage edges. The cost of (i) is therefore one new declared dependency to weigh against (ii), not a rule violation. **(ii)** Leave the trait in `pc-cli` and have `PipelineCtx` take a provider-shaped closure instead. **Lean: (i)**, with one check already done rather than assumed — `DEVIATION(27)`'s declared site is the **lazy latch**, which lives in `OnnxInpainterProvider` and does not move under (i), so §14 item 27's *"the inpainter provider in `crates/pc-cli/`"* stays true. Not ruled here: no test was drafted for it either way, and it changes a crate boundary, which is the architect's call and not this transcription's.
