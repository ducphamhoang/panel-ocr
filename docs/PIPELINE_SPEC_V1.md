...
4. **New `pc_core::device` surface: a typed, stage-scoped refusal — RULED (Orchestrator, not escalated, since this is a representation choice both plans converged on in substance and differed on only in encoding).** Both plans independently proposed the same shape — a new `DeviceRefusal` variant naming the stage that has no ratified CUDA path — differing only in whether "stage" is a typed enum (architect) or a `&'static str` (rust-engineer, offered without arguing against the enum). Ruled: the typed enum, for consistency with `pc_core::device`'s existing `Device`-enum style and because a closed set of two stages needs no string:
   ```rust
