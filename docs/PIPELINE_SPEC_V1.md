...
Third-party crates (pinned in `[workspace.dependencies]`): `serde`/`serde_json`, `toml_edit`, `image`, `imageproc`, `ndarray`, `rayon`, `clap` (derive), `thiserror`, `anyhow` (binary only), `tracing` + `tracing-subscriber`, `ort` (`=2.0.0-rc.12`, feature-gated), `reqwest` (rustls), `sha2`, `hf-hub`, `indicatif`, `regex`, `csv`. Dev: `insta` (optional, for JSON snapshots), `approx`.
...
- Score per class: `objectness * class_prob`; take the best class → `class_index`, `confidence`.
- Box decode: columns 0..4 are `cx, cy, w, h` → `x1 = cx - w/2`, `y1 = cy - h/2`, `x2 = cx + w/2`, `y2 = cy + h/2`.
- NMS: greedy, IoU threshold `0.35`, sorted by descending score, **class-agnostic** (upstream's call uses the default `agnostic=False` per-class offset trick; with 3 language classes on the same balloon this matters — see §15.1: v1 uses **class-agnostic** NMS and merges the language of the surviving box, because a balloon detected as both `ja` and `unknown` must not yield two boxes. Flagged for reviewer.)
...
3. NMS on a hand-built candidate set (three overlapping boxes with known IoUs straddling 0.35, one below `conf 0.4`) yields exactly the expected surviving boxes and classes.
...
6. `run()` with `ReplayDetector` over a recorded fixture produces a `PageDataRaw` whose JSON is byte-identical across 10 runs and across 1 vs 8 rayon threads.
...
11. On the recorded page fixture with `MockOcrEngine` returning `""` for every crop, the resulting `PageData` box tiers are locked as an `insta` JSON snapshot; the snapshot is reviewed once against a manual trace of upstream's algorithm on the same `#raw.json` (documented in `docs/GOLDEN_CALIBRATION.md`).
...
- `cleaned`: if `page.scale != 1.0`, load `original_image` and resize `combined_mask` to the original size with **nearest-neighbour**; else clone `base`. Then composite `combined_mask` over it using the mask's own alpha. Write to `dests.cleaned`. **Output mode**: match the source image's mode where possible (upstream pastes into a copy of the loaded image, so a grayscale source yields a grayscale output — this matters for the demo_bubbles goldens, which are 8-bit grayscale). Specify: if the base/original image is `L` and every `median_color` is achromatic (`r == g == b`), write `L`; otherwise write `RGB`. (§15.3 — flagged, because a coloured median on a grayscale page forces a mode promotion.)
...
- `demo_bubbles/*_raw.png` + `*_clean.png` → the **parity goldens** (7 pairs), driven by `tests/fixtures/recorded/<name>_raw_mask.png` + `<name>#raw.json` (§7.2) so no model runs in CI. Each fixture stresses a different failure mode: `square` (trivial), `handwritten` (thin strokes, small canvas), `black` (dark bubble → non-white median colour → exercises the off-white snap *not* firing), `ray`/`darkrays` (radial lines crossing the border → high border σ → exercises the improvement threshold and possibly `failed`), `spikey` (non-convex balloon → exercises growth clipping), `nightmare` (worst case → likely `failed`, exercises the "leave it alone" path).
...
15. For each of the 7 `<name>_raw.png` / `<name>_clean.png` pairs, run the full masking stage with the recorded detector output and the **default profile**, then compare `cleaned` against `<name>_clean.png`:
    - **Shape gate:** let `G = { p : raw[p] != clean[p] }` (the pixels upstream changed) and `O = { p : raw[p] != ours[p] }`. Require `IoU(G, O) ≥ 0.99`, and `O ⊆ dilate(G, 2px)` (we must not paint outside upstream's footprint by more than 2 px).
    - **Value gate:** `≥ 99.5 %` of pixels exactly equal; every remaining pixel within `±2` of the golden; global SSIM (8×8 windows, grayscale) `≥ 0.995`.
    - **Failure agreement:** for fixtures where upstream left the bubble untouched (`G` empty or tiny), we must also report `failed: true` for that region rather than painting.

    **Tolerance justification.** Every operation in this stage is either exact integer work (rect rasterization, binary dilation, edge extraction, alpha composition of 0/255 alpha) or an integer-valued reduction (median → `u8`). The only genuine float sensitivities are (i) the Weiszfeld geometric median, which can land ±1 per channel on a different integer after truncation, and (ii) the population/sample std values feeding a `<=` comparison, where a difference below `1e-12` could in principle flip a borderline candidate choice. So the *shape* should match exactly in the overwhelming majority of pixels (hence a 0.99 IoU floor rather than something loose), and the *values* should differ by at most a ±1 fill colour over a mask-sized region (hence "99.5 % exact, remainder ±2" rather than a pure SSIM threshold, which would hide a systematic ±1 fill error). A 2 px dilation slack on the footprint absorbs a single differing growth step at the smallest kernel size. These numbers are subject to the §7.3 calibration gate before the test is frozen.

16. `black_bubble` specifically: the chosen `median_color` must be dark (`max channel ≤ 40`) and the off-white snap must **not** fire — this catches a class of bug where a naive implementation always fills white.
...
**Deliberate v1 simplification:** `colored_images = true` in v1 maps to the **joint-channel RGB** path above (single `h = filter_strength`). The true `cv2.fastNlMeansDenoisingColored` — convert BGR→CIELAB, denoise `L` with `h` and `a,b` with `hColor`, convert back — is deferred to v1.5, at which point `color_filter_strength` becomes meaningful. The default profile has `colored_images = false`, so v1's default path is unaffected. Documented in §14.7, flagged in §15.7.
...
| 29 | **F2** | xtask | `calibrate-goldens` + `docs/GOLDEN_CALIBRATION.md` (**must precede freezing the §10.7(B) and §11.7(B) tests**) | simple | F1, M6, N4 |
...
5. **Noise-mask channel** — upstream derives the denoise mask from the RGBA mask's **luma**, silently losing dark-filled masks; we use the **alpha** channel.
6. **Gaussian blur** — PIL's `GaussianBlur` is a 3-pass box approximation; we use a true separable Gaussian with `sigma = radius`.
7. **Coloured NLM** — v1 uses joint-channel NLM for RGB; the Lab-split variant with `color_filter_strength` is v1.5.
8. **Mask upscale filter on export** — upstream uses bilinear only in the `denoise_mask` branch; we use nearest-neighbour uniformly.
...
1. **Class-agnostic NMS** (§8.3 step 4). Upstream's yolov5 `non_max_suppression` defaults to per-class NMS. With three *language* classes on the same balloon, per-class NMS can emit duplicate overlapping boxes for one balloon (which upstream then partially cleans up via center-merging in preprocess). I chose class-agnostic. Alternative: per-class NMS for exact parity, relying on `resolve_total_overlaps` to merge. **Recommendation: class-agnostic**, but verify against a recorded page — if box counts differ materially from upstream, switch.
2. **`MaskRefineMode::Simple` for v1** (§8.3 step 5). Confirm that shipping the koharu-style refinement (threshold/expand/dilate/clip) instead of upstream's top-k-colour + Otsu + XOR-merge refinement is acceptable for v1, given the demo_bubbles parity gates in §10.7(B) *depend on the precise mask*. **Risk:** if the Simple refinement produces a materially different precise mask, the §10.7(B) tolerances may be unreachable and F2 calibration will say so. Contingency: keep the parity gates but scope them to the recorded-fixture-derived precise mask (i.e. lock our behaviour) and defer strict upstream-image parity to v1.5 alongside `Annotation` mode.
3. **Cleaned-image colour mode when a colour median meets a grayscale page** (§10.3 step 4). I specified: stay `L` if all medians are achromatic, else promote to `RGB`. The demo_bubbles goldens are grayscale, so this rule is load-bearing for the parity gate.
4. **Tesseract deferred to v1.5.** The decisions doc lists `leptess` as the verdict for non-Japanese OCR without assigning a milestone. v1's OCR use is purely the "is this small box worth cleaning" filter, and the default profile detects `jpn`/`eng`/unknown only. I scoped v1 to manga-ocr + mock. Confirm.
5. **Fixture mapping correction** — the mock CSV/TXT fixtures define the OCR *report format* (stage 5 / E4), not OCR-filter behaviour; the filter is tested with a mock engine. The task brief mapped them to the OCR filter. Confirm the reassignment.
6. **Alpha-vs-luma noise mask** (§14.5) — this is a behaviour *fix*, and the `black_bubble` denoise output will therefore differ from upstream's. Confirm we prefer correct over identical.
7. **Joint-channel RGB NLM for `colored_images = true`** (§14.7) — confirm the deferral, given the default is `false`.
8. **Nearest-neighbour mask upscale on export** (§14.8) — confirm.
9. **`f64` vs `f32` for border statistics.** I specified `f64` throughout §10.3 (matching NumPy). The comparison `dev_i <= best_dev * (1 - t)` is sensitive; `f64` minimizes the chance of a borderline candidate flip relative to Python. Confirm no `f32` shortcuts in `border.rs`/`fit.rs`.
10. **`insta` snapshot usage.** I proposed `insta` JSON snapshots as regression locks for `#raw.json`/`#clean.json`. If the reviewer prefers hand-written assertions (more explicit, more work), say so before P5/D7 tests are drafted — snapshots are easy to accept blindly, which is a real risk under a frozen-tests rule.
