...
/// GPU-4 — the anti-vacuity check that proves the CUDA policy was applied **before**
/// `commit_from_file`, not after. Junk (non-ONNX) bytes for the model path mean the
/// session construction must fail; under a CUDA policy in a non-`cuda` build the failure
/// is the loud, run-fatal registration error from `pc_ort`'s `apply_device_policy`
/// (which runs before the model is ever committed), NOT a `commit_from_file` load
/// failure.
///
/// Gated to non-`cuda` builds for the same reason `pc-detect`'s
/// `g2_cuda_ort_boundary.rs` and `pc-ocr`'s `g3_cuda_ort_boundary.rs` gate their
/// equivalents: without `ort/cuda`, `CUDA::register` is compiled to
/// `Err(RegisterError::MissingFeature)` deterministically, independent of
...
fn a_cuda_policy_fails_on_registration_before_the_model_is_committed() {
    ...
    let model = junk_path(&root, "lama.onnx");

    let error = OnnxInpainter::from_path_with_policy(&model, &cuda_policy())
...
fn a_cpu_policy_with_the_same_junk_bytes_fails_on_the_model_load() {
    ...
    let error = OnnxInpainter::from_path_with_policy(&model, &DevicePolicy::cpu())
