//! GPU-2 (§16.47 item 4) — the LaMa stage has a refusal path only: `Stage::Inpaint` has
//! no ratified CUDA path. In a build that CAN register CUDA the seam refuses with
...
/// §16.47 item 4: LaMa has a refusal path only — a build that CAN register CUDA must
...
fn inpaint_refuses_cuda_even_in_a_build_that_can_register_it() {
    ...
    match pc_core::device::resolve(Device::Cuda, DeviceSupport::compiled()) {
        ...
        // rebuild with `--features cuda` they already used.
        Ok(_) => {
